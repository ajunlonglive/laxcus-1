 ...
 * 分布任务组件参数存取箱。<br>
 * 
 * 工作要求：从存取对象实例中提取参数和注入参数。
 * 
 * 参数在CONDUCT语句中定义，存取箱的子类根据不同的关键字解析和取出参数
 * 
 * @author scott.liang
 * @version 1.0 5/27/2018
 * @since laxcus 1.0
 */
public abstract class TaskParameterBox {

	/**
	 * 构造默认的分布任务组件参数存取箱
	 */
	protected TaskParameterBox() {
		super();
	}
	 ...
	 * @throws TaskException
	 */
	public abstract void readAll(AccessObject access) throws TaskException;
	
	/**
	 * 向存取对象注入所需的全部参数
	 * @param access 存取对象
	 */
	public abstract void writeAll(AccessObject access);

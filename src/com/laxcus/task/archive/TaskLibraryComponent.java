 ...
 * FRONT传递给CALL/DATA/WORK/BUILD节点。
 * 
 * @author scott.liang
 * @version 1.1 10/18/2019
 * @since laxcus 1.0
 */
public final class TaskLibraryComponent implements Classable, Cloneable, Serializable, Comparable<TaskLibraryComponent> { 
	...
	/**
	 * 根据传入的分布任务组件动态链接库附件，生成它的副本
	 * @param that 分布任务组件动态链接库附件
	 */
	private TaskLibraryComponent(TaskLibraryComponent that) {
		this();
		tag = that.tag;
		ware = that.ware;
		name = that.name;
	...
	/**
	 * 构造一个默认的分布任务组件动态链接库附件
	 */
	public TaskLibraryComponent() {
		super();
	}

	/**
	 * 构造分布任务组件动态链接库附件，指定参数
	 * @param tag 分布任务组件工作部件
	 * @param ware 软件名称
	 * @param name 文件名
	 * @param b 数据内容
	 */
	public TaskLibraryComponent(TaskTag tag, Naming ware, String name, byte[] b) {
		this();
		setTag(tag);
		setWare(ware);
		setName(name);
	 ...
	 * @param b 数据内容
	 */
	public TaskLibraryComponent(TaskPart part, MD5Hash sign, Naming ware, String name, byte[] b) {
		this(new TaskTag(part, sign), ware, name, b);
	}

	/**
	 * 从可类化数据读取器中解析分布任务组件动态链接库附件参数
	 * @param reader 可类化数据读取器
	 */
	public TaskLibraryComponent(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 * @param b 字节数组
	 */
	public TaskLibraryComponent(byte[] b) {
	...
	/**
	 * 生成当前实例的一个数据副本
	 * @return TaskLibraryComponent实例
	 */
	public TaskLibraryComponent duplicate() {
		return new TaskLibraryComponent(this);
	}
	...
	public boolean equals(Object that) {
		if (that == null || that.getClass() != TaskLibraryComponent.class) {
			return false;
		} else if (that == this) {
			return true;
		}
		return compareTo((TaskLibraryComponent) that) == 0;
	}
	 ...
	 */
	@Override
	public int compareTo(TaskLibraryComponent that) {
		if (that == null) {
			return 1;
		}

 ...
 * @since laxcus 1.0
 */
public abstract class HubReplyReceiveTimeoutInvoker extends CommonReplyReceiveTimeoutInvoker {

	/** 集合 **/
	private ReplyReceiveTimeoutProduct product = new ReplyReceiveTimeoutProduct();

	/**
	 * 构造设置接收异步数据超时调用器，指定命令
	 * @param cmd 应答包尺寸
	 */
	protected HubReplyReceiveTimeoutInvoker(ReplyReceiveTimeout cmd) {
		super(cmd);
	}
...
//	 * @return
//	 */
//	private ReplyReceiveTimeout createSubCommand() {
//		ReplyReceiveTimeout cmd = getCommand();
//		ReplyReceiveTimeout sub = new ReplyReceiveTimeout();
...
//					ReplyReceiveTimeout sub = createSubCommand();
...
//	}
	 ...
	 * @return
	 */
	private ReplyReceiveTimeoutItem createItem(Node node, boolean success) {
		// 生成单元
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubReplyReceiveTimeoutInvoker#launch()
	 */
	@Override
...
//		}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			// 逐一提取
			try {
				ReplyReceiveTimeoutProduct a = getObject(ReplyReceiveTimeoutProduct.class, index);
				for (ReplyReceiveTimeoutItem e : a.list()) {
...
//			ReplyReceiveTimeout cmd = (ReplyReceiveTimeout) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

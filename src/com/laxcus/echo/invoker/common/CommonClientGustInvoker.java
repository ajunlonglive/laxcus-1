 ...
 * 数据传输速率测试调用器。<br><br>
 * 
 * 以“客户机”身份，向目标站点发送流量检测命令，目标站点是“服务器”身份。
 * 
 * @author scott.liang
 * @version 1.0 8/15/2018
 * @since laxcus 1.0
 */
public class CommonClientGustInvoker extends CommonInvoker {

	/**
	 * 构造数据传输速率测试调用器，指定命令
	 * @param cmd 数据传输速率测试命令
	 */
	public CommonClientGustInvoker(Gust cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		Node to = sub.getSite();
		// 投递到目标站点
		boolean success = launchTo(to, sub);
		// 不成功，通知失败！
		if (!success) {
			replyFault();
		}
	 ...
	 * @return 传输成功返回真，否则假
	 */
	private boolean send(SwarmReflex reflux) {
		Cabin remote = reflux.getSource();

		TrafficProduct product = null;

		// 建立异步通信
		CastToken token = start(remote);
		// 获得标识，上传数据（当前是客户机，接收方是服务器）
		if (token != null) {
			product = upload(token);
		}
		// 无论异步通信是否成功，都要通知对方，关闭连接
		exit(remote, product);

		// 向WATCH站点反馈处理结果，成功或者失败！
	...
	private TrafficProduct upload(CastToken token) {
		...
		// 以下三个参数：1. 发送间隔；2. 发送单元数；3. 发送模式（并行）。设置成最大限额
		...
		while (seek < end) {
			...
			if (!success) {
				Logger.error(this, "upload", "cannot send data! receive timeout:%d", client.getReceiveTimeout());
				break;
			}
			// 移到下标
			seek += len;
		...
		success = (seek == end);
		// 保存参数
		TrafficProduct product = new TrafficProduct(success, getLocal(), cmd.getSite());

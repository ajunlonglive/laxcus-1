 ...
 * @since laxcus 1.0
 */
public abstract class HubReloadSecurityPolicyInvoker extends CommonReloadSecurityPolicyInvoker {

	/** 集合 **/
	private ReloadSecurityPolicyProduct product = new ReloadSecurityPolicyProduct();

	/**
	 * 构造重新设置节点的安全策略命令调用器，指定命令
	 * @param cmd 重新设置节点的安全策略命令
	 */
	protected HubReloadSecurityPolicyInvoker(ReloadSecurityPolicy cmd) {
		super(cmd);
	}
	 ...
	 * @return 副本命令
	 */
	private ReloadSecurityPolicy createSubCommand() {
		ReloadSecurityPolicy cmd = getCommand();
...
//					ReloadSecurityPolicy sub = createSubCommand();
...
//	}
	...
	public boolean launch() {
		...
		if (cmd.isAll()) {
			...
			// 更新自己
			ReloadSecurityPolicyItem libItem = reload();
			product.add(libItem);
		} else {
//			// 逐一检查，匹配才处理
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				ReloadSecurityPolicyProduct a = getObject(ReloadSecurityPolicyProduct.class, index);
				for (ReloadSecurityPolicyItem e : a.list()) {
					// 如果单元已经存在，并且成功时，删除旧单元。注意，必须是成功！
...
//			ReloadSecurityPolicy cmd = (ReloadSecurityPolicy) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

...
/**
 * 管理站点的强制重新发布自定义JAR包调用器。
 * 这个调用器提供给TOP/HOME站点使用。
 * 
 * @author scott.liang
 * @version 1.0 11/13/2017
 * @since laxcus 1.0
 */
public abstract class HubReloadCustomInvoker extends CommonWatchShareInvoker {

	/** 集合 **/
	private ReloadCustomProduct product = new ReloadCustomProduct();

	/**
	 * 构造管理站点的强制重新发布自定义JAR包调用器，指定命令
	 * @param cmd 强制重新发布自定义JAR包命令
	 */
	protected HubReloadCustomInvoker(ReloadCustom cmd) {
		super(cmd);
	}
	 ...
	 * @return 新命令
	 */
	private ReloadCustom createSubCommand() {
		ReloadCustom cmd = getCommand();
...
//					ReloadCustom sub = createSubCommand();
...
//			ReloadCustom cmd = (ReloadCustom) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				ReloadCustomProduct a = getObject(ReloadCustomProduct.class, index);
				for (ReloadCustomItem e : a.list()) {
					// 如果单元已经存在，并且是成功时，删除旧单元。注意，必须是成功！

 ...
 * @since laxcus 1.0
 */
public abstract class HubScanCommandStackInvoker extends CommonScanCommandStackInvoker {

	/** 集合 **/
	private ScanCommandStackProduct product = new ScanCommandStackProduct();

	/**
	 * 构造扫描堆栈命令调用器，指定命令
	 * @param cmd 扫描堆栈命令
	 */
	protected HubScanCommandStackInvoker(ScanCommandStack cmd) {
		super(cmd);
	}
	...
	//	 * @return 副本命令
	//	 */
	//	private ScanCommandStack createSubCommand() {
	//		ScanCommandStack cmd = getCommand();
	//		ScanCommandStack sub = new ScanCommandStack();
...
//					ScanCommandStack sub = createSubCommand();
...
//	}
	...
	public boolean launch() {
		...
		if (cmd.isAll()) {
			...
			// 更新自己
			ScanCommandStackItem libItem = reload();
			product.add(libItem);
		} else {
			//			// 逐一检查，匹配才处理
			//			for (Node node : cmd.getSites()) {
			...
			//					ScanCommandStack sub = createSubCommand();
			//					sub.addSite(node); // 保存本节点
			//					// 生成命令单元保存
			//					CommandItem item = new CommandItem(node, sub);
			//					array.add(item);
			//				} else {
			//					// 找到HOME/BANK站点，把站点地址分配到它的下属
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				ScanCommandStackProduct a = getObject(ScanCommandStackProduct.class, index);
				for (ScanCommandStackItem e : a.list()) {
	...
	//			}
	//		}
	//		// 3. 把工作站点分配给HOME/BANK站点的命令单元
	//		for (CommandItem e : temps) {
	//			ScanCommandStack cmd = (ScanCommandStack) e.getCommand();
	//			cmd.addSite(job);
	//		}
	//		// 4. 以上空时，把生成单元保存到输出数据
	//		if (empty) {
	...
	//	/**
	//	 * 相对于当前管理节点，判断是当前管理节点的子节点的子节点。如果TOP则是，HOME/BANK则不是。<br>

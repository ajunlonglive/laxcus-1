 ...
 * @since laxcus 1.0
 */
public abstract class HubLeastDiskInvoker extends CommonWatchShareInvoker {

	/** 反馈结果集合 **/
	private LeastDiskProduct product = new LeastDiskProduct();

	/**
	 * 构造默认的管理节点的节点最小磁盘空间限制调用器，指定命令
	 * @param cmd 管理节点的节点最小磁盘空间限制
	 */
	protected HubLeastDiskInvoker(LeastDisk cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private LeastDisk createSubCommand() {
		LeastDisk cmd = getCommand();
...
//					LeastDisk sub = createSubCommand();
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				LeastDiskProduct that = getObject(LeastDiskProduct.class, index);
				for (LeastDiskItem e : that.list()) {
...
//			LeastDisk cmd = (LeastDisk) e.getCommand();
//			cmd.addSite(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

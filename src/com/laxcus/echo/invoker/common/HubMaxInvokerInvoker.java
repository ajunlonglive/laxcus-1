 ...
 * @since laxcus 1.0
 */
public abstract class HubMaxInvokerInvoker extends CommonMaxInvokerInvoker {

	/** 集合 **/
	private MaxInvokerProduct product = new MaxInvokerProduct();

	/**
	 * 构造设置调用器数目调用器，指定命令
	 * @param cmd 调用器数目
	 */
	protected HubMaxInvokerInvoker(MaxInvoker cmd) {
		super(cmd);
	}
...
//	 * @return
//	 */
//	private MaxInvoker createSubCommand() {
//		MaxInvoker cmd = getCommand();
//		MaxInvoker sub = cmd.duplicate();
...
//					MaxInvoker sub = createSubCommand();
...
//	}
	 ...
	 * @return
	 */
	private MaxInvokerItem createItem(Node node, boolean success) {
		// 生成单元
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubMaxInvokerInvoker#launch()
	 */
	@Override
...
//		}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			// 逐一提取
			try {
				MaxInvokerProduct a = getObject(MaxInvokerProduct.class, index);
				for (MaxInvokerItem e : a.list()) {
...
//			MaxInvoker cmd = (MaxInvoker) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

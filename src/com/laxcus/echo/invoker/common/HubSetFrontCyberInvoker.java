 ...
 * @since laxcus 1.0
 */
public abstract class HubSetFrontCyberInvoker extends CommonWatchShareInvoker {

	/** 反馈结果集合 **/
	private VirtualCyberProduct product = new VirtualCyberProduct();

	/**
	 * 构造默认的管理节点的设置FRONT在线用户虚拟空间参数调用器，指定命令
	 * @param cmd 管理节点的设置FRONT在线用户虚拟空间参数
	 */
	protected HubSetFrontCyberInvoker(SetFrontCyber cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private SetFrontCyber createSubCommand() {
...
//					SetFrontCyber sub = createSubCommand();
...
//	}
		...
		for (int index : keys) {
			...
			try {
				...
				for (VirtualCyberItem e : that.list()) {
					// 如果单元已经存在，并且是成功，删除旧单元，保存新单元。注意，是成功！
	...
	//			SetFrontCyber cmd = (SetFrontCyber) e.getCommand();
	//			cmd.add(job);
	//		}
	//		// 4. 以上空时，把生成单元保存到输出数据
	//		if (empty) {

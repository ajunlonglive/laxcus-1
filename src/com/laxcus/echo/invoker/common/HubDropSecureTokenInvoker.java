 ...
 * @since laxcus 1.0
 */
public abstract class HubDropSecureTokenInvoker extends CommonDropSecureTokenInvoker {

	/** 集合 **/
	private DropSecureTokenProduct product = new DropSecureTokenProduct();

	/**
	 * 构造删除密钥令牌调用器，指定命令
	 * @param cmd 删除密钥令牌
	 */
	protected HubDropSecureTokenInvoker(DropSecureToken cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private DropSecureToken createSubCommand() {
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubDropSecureTokenInvoker#launch()
	 */
	@Override
	public boolean launch() {
		...
		// 如果是全部，直接更新
		if (cmd.isAll()) {
			DropSecureToken sub = createSubCommand();
			// 逐一分配给下属
			for (Node node : sites) {
...
//		}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			// 逐一提取
			try {
				DropSecureTokenProduct a = getObject(DropSecureTokenProduct.class, index);
				for (DropSecureTokenItem e : a.list()) {

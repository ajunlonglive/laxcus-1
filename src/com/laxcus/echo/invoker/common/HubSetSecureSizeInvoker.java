 ...
 * @since laxcus 1.0
 */
public abstract class HubSetSecureSizeInvoker extends CommonSetSecureSizeInvoker {

	/** 集合 **/
	private SetSecureSizeProduct product = new SetSecureSizeProduct();

	/**
	 * 构造设置对称密钥长度调用器，指定命令
	 * @param cmd 设置对称密钥长度
	 */
	protected HubSetSecureSizeInvoker(SetSecureSize cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private SetSecureSize createSubCommand() {
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubSetSecureSizeInvoker#launch()
	 */
	@Override
	public boolean launch() {
		...
		// 如果是全部，直接更新
		if (cmd.isAll()) {
			SetSecureSize sub = createSubCommand();
			// 逐一分配给下属
			for (Node node : sites) {
...
//		}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			// 逐一提取
			try {
				SetSecureSizeProduct a = getObject(SetSecureSizeProduct.class, index);
				for (SetSecureSizeItem e : a.list()) {

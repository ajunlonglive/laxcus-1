 ...
 * @since laxcus 1.0
 */
public abstract class HubOpenTiggerInvoker extends CommonWatchShareInvoker {
	 ...
	 */
	protected HubOpenTiggerInvoker(OpenTigger cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private OpenTigger createSubCommand() {
		OpenTigger cmd = getCommand();
...
//					OpenTigger sub = createSubCommand();
		...
		for (int index : keys) {
			...
			try {
				...
				for (ProcessTiggerItem e : that.list()) {
					// 如果单元已经存在，并且是成功，删除旧单元，保存新单元。注意，必须是成功！
...
//			OpenTigger cmd = (OpenTigger) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

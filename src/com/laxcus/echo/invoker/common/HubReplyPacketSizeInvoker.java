 ...
 * @since laxcus 1.0
 */
public abstract class HubReplyPacketSizeInvoker extends CommonReplyPacketSizeInvoker {

	/** 集合 **/
	private ReplyPacketSizeProduct product = new ReplyPacketSizeProduct();

	/**
	 * 构造设置应答包尺寸调用器，指定命令
	 * @param cmd 应答包尺寸
	 */
	protected HubReplyPacketSizeInvoker(ReplyPacketSize cmd) {
		super(cmd);
	}
...
//	 * @return
//	 */
//	private ReplyPacketSize createSubCommand() {
//		ReplyPacketSize cmd = getCommand();
//		ReplyPacketSize sub = new ReplyPacketSize();
...
//					ReplyPacketSize sub = createSubCommand();
...
//	}
	 ...
	 * @return
	 */
	private ReplyPacketSizeItem createItem(Node host, boolean success) {
		ReplyPacketSizeItem item = new ReplyPacketSizeItem(host, success);
		if (success) {
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubReplyPacketSizeInvoker#launch()
	 */
	@Override
...
//		}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			// 逐一提取
			try {
				ReplyPacketSizeProduct a = getObject(ReplyPacketSizeProduct.class, index);
				for (ReplyPacketSizeItem e : a.list()) {
					// 如果单元已经存在，并且是成功时，删除旧单元。注意，是成功！
...
//			ReplyPacketSize cmd = (ReplyPacketSize) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

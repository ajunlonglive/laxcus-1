 ...
 * @since laxcus 1.0
 */
public abstract class HubMostCPUInvoker extends CommonWatchShareInvoker {

	/** 反馈结果集合 **/
	private MostCPUProduct product = new MostCPUProduct();

	/**
	 * 构造默认的管理节点的最大CPU使用率限制调用器，指定命令
	 * @param cmd 管理节点的最大CPU使用率限制
	 */
	protected HubMostCPUInvoker(MostCPU cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private MostCPU createSubCommand() {
		MostCPU cmd = getCommand();
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				MostCPUProduct that = getObject(MostCPUProduct.class, index);
				for (MostCPUItem e : that.list()) {
					// 如果单元已经存在，并且是成功时，删除旧单元，保存新单元。注意，必须是成功！
...
//			MostCPU cmd = (MostCPU) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

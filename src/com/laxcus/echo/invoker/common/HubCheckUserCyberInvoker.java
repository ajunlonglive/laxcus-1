 ...
 * @since laxcus 1.0
 */
public abstract class HubCheckUserCyberInvoker extends CommonWatchShareInvoker {

	/** 反馈结果集合 **/
	private UserCyberProduct product = new UserCyberProduct();

	/**
	 * 构造默认的管理节点的设置成员虚拟空间参数调用器，指定命令
	 * @param cmd 管理节点的设置成员虚拟空间参数
	 */
	protected HubCheckUserCyberInvoker(CheckUserCyber cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private CheckUserCyber createSubCommand() {
		CheckUserCyber cmd = getCommand();
		cmd.setQuick(true);
...
//					CheckUserCyber sub = createSubCommand();
...
//					if (!success) product.add(node, null);
//				}
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			if (!isSuccessObjectable(index)) {
				...
				product.add(node, null);
				continue;
			}
			try {
				UserCyberProduct that = getObject(UserCyberProduct.class, index);
				for (UserCyberItem e : that.list()) {
					// 如果单元已经存在，而且瞬时记录有效，删除旧单元，保存新单元
					if (product.contains(e) && e.getMoment() != null) {
						product.remove(e);
					}
					// 保存新单元
					product.add(e);
...
//			CheckUserCyber cmd = (CheckUserCyber) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

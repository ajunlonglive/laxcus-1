 ...
 * @since laxcus 1.0
 */
public abstract class HubLeastMemoryInvoker extends CommonWatchShareInvoker {

	/** 反馈结果集合 **/
	private LeastMemoryProduct product = new LeastMemoryProduct();

	/**
	 * 构造默认的管理节点的节点最小内存限制调用器，指定命令
	 * @param cmd 管理节点的节点最小内存限制
	 */
	protected HubLeastMemoryInvoker(LeastMemory cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private LeastMemory createSubCommand() {
		LeastMemory cmd = getCommand();
...
//					LeastMemory sub = createSubCommand();
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				LeastMemoryProduct that = getObject(LeastMemoryProduct.class, index);
				for (LeastMemoryItem e : that.list()) {
...
//			LeastMemory cmd = (LeastMemory) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

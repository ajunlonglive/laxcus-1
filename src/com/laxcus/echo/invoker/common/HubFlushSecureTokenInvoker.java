 ...
 * @since laxcus 1.0
 */
public abstract class HubFlushSecureTokenInvoker extends CommonFlushSecureTokenInvoker {

	/** 集合 **/
	private FlushSecureTokenProduct product = new FlushSecureTokenProduct();

	/**
	 * 构造FlushSecureToken命令调用器，指定命令
	 * @param cmd FlushSecureToken命令
	 */
	protected HubFlushSecureTokenInvoker(FlushSecureToken cmd) {
		super(cmd);
	}
	 ...
	 * @return
	 */
	private FlushSecureToken createSubCommand() {
...
//					FlushSecureToken sub = createSubCommand();
...
//	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.echo.invoke.common.HubFlushSecureTokenInvoker#launch()
	 */
	@Override
...
//			}
	...
	public boolean ending() {
		...
		for (int index : keys) {
			...
			try {
				FlushSecureTokenProduct a = getObject(FlushSecureTokenProduct.class, index);
				for (FlushSecureTokenItem e : a.list()) {
					// 如果单元已经存在，删除旧单元
...
//			FlushSecureToken cmd = (FlushSecureToken) e.getCommand();
//			cmd.add(job);
//		}
//		// 4. 以上空时，把生成单元保存到输出数据
//		if (empty) {

 ...
 * @since laxcus 1.0
 */
public class WorkTalkAskInvoker extends WorkInvoker {

	/**
	 * 构造分布任务组件远程交互调用器，指定命令
	 * @param cmd 分布任务组件远程交互
	 */
	public WorkTalkAskInvoker(TalkAsk cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		if (invoker != null) {
			if (Laxkit.isClassFrom(invoker, WorkConductToInvoker.class)) {
				WorkConductToInvoker sub = (WorkConductToInvoker) invoker;
				reply = sub.ask(quest);
			}
		}

 ...
 * 刷新发布的任务组件调用器。<br><br>
 * 
 * 通知管理池，去ACCOUNT站点下载新的组件，包括分布任务组件、码位计算、快捷组件。
 * 
 * @author scott.liang
 * @version 1.0 3/13/2018
 * @since laxcus 1.0
 */
public class WorkRefreshPublishInvoker extends WorkInvoker {

	/**
	 * 构造刷新发布的任务组件调用器，指定命令
	 * @param cmd 刷新发布的任务组件
	 */
	public WorkRefreshPublishInvoker(RefreshPublish cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		if (siger != null) {
			...
			WorkCommandPool.getInstance().admit(sub);
		}

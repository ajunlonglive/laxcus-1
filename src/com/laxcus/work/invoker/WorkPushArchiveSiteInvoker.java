 ...
 * 主动推送ARCHIVE站点命令调用器。
 * 
 * 收到HOME站点推送的ARCHIVE站点后，系统将启动TakeTaskTag命令，去检查分布组件被更新。
 ...
 * @since laxcus 1.0
 */
public class WorkPushArchiveSiteInvoker extends WorkInvoker {

	/**
	 * 构造主动推送ARCHIVE站点命令调用器，指定命令
	 * @param cmd 主动推送ARCHIVE站点命令
	 */
	public WorkPushArchiveSiteInvoker(PushAccountSite cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (Siger siger : cmd.list()) {
			// 签名有效
			boolean success = StaffOnWorkPool.getInstance().allow(siger);
			// 保存账号和ARCHIVE站点地址
			if (success) {
		...
		// 检查用户级分布任务组件、码位计算器、快捷组件
		boolean success = (array.size() > 0);
		if (success) {
			for (Siger siger : array) {
				StaffOnWorkPool.getInstance().loadTasks(siger);
//				StaffOnWorkPool.getInstance().loadScaler(siger);
//				StaffOnWorkPool.getInstance().loadSwift(siger);
			}
		}

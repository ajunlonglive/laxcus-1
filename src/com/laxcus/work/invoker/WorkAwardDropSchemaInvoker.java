 ...
 * @since laxcus
 */
public class WorkAwardDropSchemaInvoker extends WorkInvoker {

	/**
	 * 构造授权删表命令调用器，指定命令
	 * @param cmd 授权删除表
	 */
	public WorkAwardDropSchemaInvoker(AwardDropSchema cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 找到全部数据表
		List<Space> spaces = StaffOnWorkPool.getInstance().findSpaces(fame);
		// 逐一删除
		for (Space space : spaces) {
			boolean success = StaffOnWorkPool.getInstance().dropTable(space);

 ...
 * @since laxcus 1.0
 */
public class WorkDeployTableInvoker extends WorkInvoker {

	/**
	 * 构造部署数据表调用器，指定命令
	 * @param cmd
	 */
	public WorkDeployTableInvoker(DeployTable cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 判断已经存在资源引用
		boolean success = StaffOnWorkPool.getInstance().hasRefer(refer.getUsername());
		if (!success) {
			success = StaffOnWorkPool.getInstance().create(refer);
		}
		// 建表
		if (success) {
			success = StaffOnWorkPool.getInstance().createTable(table);
		}

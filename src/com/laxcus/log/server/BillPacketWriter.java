 ...
 * @since laxcus 1.0
 */
final class BillPacketWriter implements PacketInvoker {
	 ...
	 * @param root
	 */
	public BillPacketWriter(File root) {
		this();
		setRoot(root);
	}
	...
	public Packet invoke(Packet packet) {
		...
		resp.addMessage(MessageKey.SPEAK, "reply bill");
		return resp;
	}
	...
	private File mkfile(SocketHost source) {
		...
		for (int index = 1; index < Integer.MAX_VALUE; index++) {
			String suffix = String.format("%s(%d).bil", today, index);
			File file = new File(logPath, suffix);
			//返回条件：1，文件不存在. 2，文件长度在范围内
			if (!file.exists()) {
				return file;
			} else if (file.length() < BillPacketWriter.MAX_LOGSIZE) {
				return file;
			}
		}
		// 返回空值，可能性几乎没有
	 ...
	 * @return 成功返回真，否则假
	 */
	private boolean writeBill(SocketHost source, byte[] logs) {

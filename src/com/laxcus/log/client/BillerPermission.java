 ...
 * 操作操作许可 <br>
 * 
 * 在运行过程中，对操作的方法调用时的检查<br><br>
 * 
 * 在各节点 site.policy 文件中的操作格式：<br>
 * <1> permission com.laxcus.log.client.BillerPermission "using.*"; 允许所有用户使用操作 <br>
 * <2> permission com.laxcus.log.client.BillerPermission "using.debug|using.info|using.warning|using.error|using.fatal"; 允许所有用户使用其中一种操作 <br>
 * <3> permission com.laxcus.log.client.BillerPermission "using.*", "用户签名（64个16进制字符，忽略大小写）"; 允许某个用户使用全部操作 <br><br>
 * <4> permission com.laxcus.log.client.BillerPermission "using.debug|using.info|using.warning|using.error|using.fatal", "用户签名（64个16进制字符，忽略大小写）,用户签名"; 允许某个用户使用其中一种或者几种操作，多个用户签名用逗号隔开。 <br><br> 
 * <5> permission com.laxcus.log.client.BillerPermission "using.debug|using.info|using.warning|using.error|using.fatal", "用户明文名称,用户明文名称"; 允许多个用户名称一起，中间用逗号隔开 <br><br>
 * 
 * @author scott.liang
 * @version 1.0 1/6/2020
 * @since laxcus 1.0
 */
public class BillerPermission extends BasicPermission {
	 ...
	 * @param name 目标名称
	 */
	public BillerPermission(String name) {
		super(name);
	}
	 ...
	 */
	public BillerPermission(String name, String actions) {
		super(name, actions);
		setIssuers(actions);
	}
	...
	public boolean equals(Object permission) {
		...
		if (!(permission instanceof BillerPermission)) {
			return false;
		}

		BillerPermission that = (BillerPermission) permission;
	...
	public PermissionCollection newPermissionCollection() {
		return new BillerPermissionCollection();
	}
}
 ...
 * @since laxcus 1.0
 */
public final class TigClient extends VirtualThread {

	/** 操作文件后缀 **/
	private final static String suffix = ".tig";
	 ...
	 * @param e
	 */
	public void setTigConfigure(TigConfigure e) {
		configure = e;
	 ...
	 * @return
	 */
	public TigConfigure getTigConfigure() {
		return configure;
	}
...
//	}
	...
	private boolean choose() {
		...
		for (int index = 1; index < Integer.MAX_VALUE; index++) {
			String name = String.format("%s(%d)%s", today, index, TigClient.suffix);
	 ...
	 * @return
	 */
	public boolean load(TigConfigure config, SiteHost endpoint) {		
		boolean success = false;
		if (config.isNotSend() || config.isSendToBuffer()) {
			setTigConfigure(config);
			success = true;
			...
			// 如果是服务器模式，必须有目标服务器地址
			if (endpoint == null) {
				Logger.error("TigClient.load, tig service site is null!");
				return false;
			}
			...
			throw new IllegalArgumentException("illegal tig mode type!");
		}
	...
	private void sendToServer(String log) {
		...
		Mark cmd = new Mark(Ask.NOTIFY, Ask.FLUSH_TIG);
	...
	void push(int level, String log) {
		...
		String text = String.format("%s: %s %s", TigType.getText(level), now(), log);
		push(text);
	}
	...
	//				TigLevel.getText(level), now(), signer, log);
	//		push(text);
	//	}

 ...
 * @since laxcus 1.0
 */
public class MemberFull extends Command {

	private static final long serialVersionUID = -4747214901862859428L;
	 ...
	 */
	public MemberFull(int maxPersons, int realPersons) {
		this();
	...
	/**
	 * 生成用户虚拟空间耗尽命令的数据副本
	 * @param that MemberFull实例
	 */
	private MemberFull(MemberFull that) {
		super(that);
		site = that.site;
	...
	/**
	 * 构造用户虚拟空间耗尽，指定站点地址和数据表名
	 * @param site 站点地址
	 */
	public MemberFull(Node site) {
		this();
		setSite(site);
	}

	/**
	 * 从可类化数据读取器中解析用户虚拟空间耗尽命令
	 * @param reader 可类化数据读取器
	 */
	public MemberFull(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 */
	@Override
	public MemberFull duplicate() {
		return new MemberFull(this);
	}

 ...
 * @since laxcus 1.0
 */
public class FrontMissing extends Command {

	private static final long serialVersionUID = 2656399214218686259L;
	 ...
	 */
	public FrontMissing(int maxPersons, int realPersons) {
		this();
	...
	/**
	 * 生成FRONT用户虚拟空间不足命令的数据副本
	 * @param that FrontMissing实例
	 */
	private FrontMissing(FrontMissing that) {
		super(that);
		site = that.site;
	...
	/**
	 * 构造FRONT用户虚拟空间不足，指定站点地址和数据表名
	 * @param site 站点地址
	 */
	public FrontMissing(Node site) {
		this();
		setSite(site);
	}

	/**
	 * 从可类化数据读取器中解析FRONT用户虚拟空间不足命令
	 * @param reader 可类化数据读取器
	 */
	public FrontMissing(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 */
	@Override
	public FrontMissing duplicate() {
		return new FrontMissing(this);
	}

 ...
 * 设置应答包传输模式。<br>
 * 
 * 对应ReplyTransfer.SERIAL_TRANSFER 和 ReplyTransfer.PARALLEL_TRANSFER <br>
 * 
 ...
 * @since laxcus 1.0
 */
public class ReplyPacketMode extends Command {

	private static final long serialVersionUID = -3233333178804186027L;
	 ...
	 * 构造默认的设置应答包传输模式命令
	 */
	public ReplyPacketMode() {
		super();
		local = false;
	}

	/**
	 * 从可类化数据读取器中解析设置应答包传输模式命令
	 * @param reader 可类化数据读取器
	 */
	public ReplyPacketMode(ClassReader reader) {
		this();
		resolve(reader);
	}
	...
	/**
	 * 生成设置应答包传输模式命令的数据副本
	 * @param that ReplyPacketMode实例
	 */
	private ReplyPacketMode(ReplyPacketMode that) {
		super(that);
	 ...
	 */
	@Override
	public ReplyPacketMode duplicate() {
		return new ReplyPacketMode(this);
	}

 ...
 * @since laxcus 1.0
 */
public class PushGateSite extends Command {
	...
	/**
	 * 构造默认和私有的推送GATE站点到ENTRANCE站点命令
	 */
	private PushGateSite() {
		super();
	}
	 ...
	 * @param that
	 */
	private PushGateSite(PushGateSite that) {
		super(that);
		no = that.no;
		inner = that.inner;
		outer =that.outer;
	 ...
	 * @param no 编号
	 */
	public PushGateSite(Node inner, Node outer, int no) {
		this();
		setInner(inner);
		setOuter(outer);
		setNo(no);
	 ...
	 */
	public PushGateSite(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 */
	@Override
	public PushGateSite duplicate() {
		return new PushGateSite(this);
	}

 ...
 * @since laxcus 1.0
 */
public class HashSeekUserAreaInvoker extends HashInvoker {

	/**
	 * 构造检索用户分布区域调用器，指定命令
	 * @param cmd 检索用户分布区域命令
	 */
	public HashSeekUserAreaInvoker(SeekUserArea cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (Siger siger : sigers) {
			boolean success = StaffOnHashPool.getInstance().hasSiger(siger);
			if (success) {

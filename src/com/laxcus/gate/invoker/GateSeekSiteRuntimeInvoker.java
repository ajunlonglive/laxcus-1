 ...
 * @since laxcus 1.0
 */
public class GateSeekSiteRuntimeInvoker extends CommonSeekSiteRuntimeInvoker {

	/**
	 * 构造诊断节点运行时状态调用器，指定命令
	 * @param cmd 诊断节点运行时状态
	 */
	public GateSeekSiteRuntimeInvoker(SeekSiteRuntime cmd) {
		super(cmd);
	}
	...
	protected int getRegisterMembers() {
		return StaffOnGatePool.getInstance().size();
	}
	...
	protected int getOnlineMembers() {
		return FrontOnGatePool.getInstance().getMembers();
	}

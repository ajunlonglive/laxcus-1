 ...
 * 授权删表调用器。<br>
 * 
 * 删表命令由HOME站点发送，BUILD站点无条件接受和执行删除。
 * 
 * @author scott.liang
 * @version 1.0 5/23/2013
 * @since laxcus 1.0
 */
public class BuildAwardDropTableInvoker extends BuildInvoker {

	/**
	 * 构造授权删表调用器，指定命令
	 * @param cmd 授权删表命令
	 */
	public BuildAwardDropTableInvoker(AwardDropTable cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 判断数据表获得授权许可
		boolean success = StaffOnBuildPool.getInstance().allow(space);
		// 执行删除
		if (success) {
			success = StaffOnBuildPool.getInstance().dropTable(space);
		}
		// 重新注册
		if (success) {

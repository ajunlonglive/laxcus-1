 ...
 * @since laxcus 1.0
 */
public class BuildPushArchiveSiteInvoker extends BuildInvoker {

	/**
	 * 构造主动推送ACCOUNT站点命令调用器，指定命令
	 * @param cmd 主动推送ACCOUNT站点命令
	 */
	public BuildPushArchiveSiteInvoker(PushAccountSite cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (Siger siger : cmd.list()) {
			// 签名有效
			boolean success = StaffOnBuildPool.getInstance().allow(siger);
			// 保存账号和ACCOUNT站点地址
			if (success) {
...
//				StaffOnBuildPool.getInstance().loadScaler(siger);
			}
		}

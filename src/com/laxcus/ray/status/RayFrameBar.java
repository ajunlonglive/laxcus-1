 ...
 * @since laxcus 1.0
 */
final class RayFrameBar extends RayPanel {
...
//	}
	...
	/** 桌面 **/
	private RaySelectFrameListener frameListener;
	...
	/** 左侧按纽 **/
	private RayFrameArrowButton cmdLeft;

	/** 右侧按纽 **/
	private RayFrameArrowButton cmdRight;
...
//		cmdRight.addMouseListener(new ArrowMouseTracker());
	...
	private int doButtons(int barWidth) {
		...
		do {
			// 记录间隔
			if (width > 0) {
				width += RayFrameBar.FRAMEBUTTON_GAP;
			}
...
//			int buttonsWidth = buttons * DesktopFrameBar.FRAMEBUTTON_WIDTH + (buttons - 1) * DesktopFrameBar.FRAMEBUTTON_GAP;
			...
			int buttonsWidth = (buttons * butWidth) + ((buttons - 1) * RayFrameBar.FRAMEBUTTON_GAP);
...
//			int buttonsWidth = buttons * getButtonDefaultWidth() + (buttons - 1) * DesktopFrameBar.FRAMEBUTTON_GAP;
	 ...
	 * @param s
	 */
	public void setSelectFrameListener(RaySelectFrameListener s) {
		frameListener = s;
	}
	 ...
	 * @return
	 */
	public RaySelectFrameListener getSelectFrameListener() {

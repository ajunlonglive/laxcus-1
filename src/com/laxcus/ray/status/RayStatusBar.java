 ...
 * @since laxcus 1.0
 */
public class RayStatusBar extends RayPanel implements FrameBar, PlatformStatusBar {
	...
	//		// 图标资源，在JAR文档中
	//		ResourceLoader rs = new ResourceLoader("conf/ray/image/window/status/");
	 ...
	 * 设置管理员图标
	 */
	public void setAdministratorIconX() {
...
//		monitorThread.addEvent(thread);
	...
	private void doFlash(int who) {
		...
		// 判断处于连网状态
		boolean login = RayLauncher.getInstance().isLogined();
		// 选择图标
		if (login) {
			if (who == RayFlashTag.FLASH_START) {
				icon = flashLight;
			} else if (who == RayFlashTag.FLASH_NEXT) {
				icon = flashDrak;
			} else if (who == RayFlashTag.FLASH_STOP) {
				// 连接有效图标
				icon = connect;
			}
		} else {
	...
	public void flash() {
		...
		// 图标编号
		int[] icons = new int[] { RayFlashTag.FLASH_START, RayFlashTag.FLASH_NEXT };
		...
		// 多延时200毫秒，效果更好！
		FlashThread last = new FlashThread(RayFlashTag.FLASH_STOP);
...
//	}
	...
	private void createImages() {
		...
		// unbody = (ImageIcon) UIManager.getIcon("StatusBar.UnbodyIcon");
...
//		FontKit.setToolTipText(lblTub, UIManager.getString("StatusBar.TubTitle")); // findCaption("ToolTip/Tub/title"));
	...
	private JPanel createRightBar() {
		JLabel[] labels = new JLabel[] { lblTray, lblInvoker, lblSound, lblHub };
		...
		left.setLayout(new GridLayout(1, 2, 8, 0));
...
//		unuserTip = UIManager.getString("StatusBar.UnuserTitle");
		...
		// 设置句柄
		RayTrayManager tm = (RayTrayManager) PlatformKit.getTrayManager();
		if (tm != null) {
			tm.setTrayController(trayController);
		}
		
		//		// 使到计算器句柄
		//		Timer timer = RayLauncher.getInstance().getTimer();
		//		// 间隔1秒钟触发
...
//		// 边缘容器
//		SocketHost host = RayLauncher.getInstance().getTubHost();
...
//		}
		...
		public void process() {
			...
			JComponent[] members = new JComponent[] { lblInvoker,
					lblSound, lblHub, lblTray, lblTime };
...
//	}
	 ...
	 * @param l
	 */
	public void setSelectFrameListener(RaySelectFrameListener l) {

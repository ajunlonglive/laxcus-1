 ...
 * @since laxcus 1.0
 */
public class RaySeekUserAreaInvoker extends RayInvoker {

	/**
	 * 构造检索用户分布区域调用器，指定命令
	 * @param cmd 检索用户分布区域
	 */
	public RaySeekUserAreaInvoker(SeekUserArea cmd) {
		super(cmd);
	}
	...
	private boolean isRegisterOnline(Node node) {
		return node.isCall() && RayFrontMemberBasket.getInstance().contains(node);
	}
	...
	private boolean isOnline(Node node) {
		return node.isGate()
				|| (node.isCall() && RayFrontMemberBasket.getInstance().contains(node));
	}

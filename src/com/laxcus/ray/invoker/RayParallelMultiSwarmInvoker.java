 ...
 * @since laxcus 1.0
 */
public class RayParallelMultiSwarmInvoker extends RayInvoker {

	/**
	 * 构造并行流量测试调用器，指定命令
	 * @param cmd 并行流量测试命令
	 */
	public RayParallelMultiSwarmInvoker(ParallelMultiSwarm cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (int i = 0; i < count; i++) {
			...
			real.setSerial(i + 1);
			RayMultiSwarmInvoker invoker = new RayMultiSwarmInvoker(real);

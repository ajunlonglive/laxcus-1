 ...
 * @since laxcus 1.0
 */
public abstract class RayDeployCloudPackageInvoker extends RayInvoker {

	/**
	 * 构造发布系统软件包调用器
	 * @param cmd 应用包
	 */
	protected RayDeployCloudPackageInvoker(DeployCloudPackage cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 找到全部ACCOUNT站点
		List<Node> sites = SiteOnRayPool.getInstance().list(SiteTag.ACCOUNT_SITE);
		// 空集合，弹出错误
		if (sites.isEmpty()) {

 ...
 * @since laxcus 1.0
 */
public class RayShiftSeekSiteRuntimeInvoker extends RayInvoker {

	/**
	 * 构造转发检查被WATCH站点监视的站点状态，指定转发命令
	 * @param shift 转发检查被WATCH站点监视的站点状态
	 */
	public RayShiftSeekSiteRuntimeInvoker(ShiftSeekSiteRuntime shift) {
		super(shift);
		// 不要记录!
		setTigger(false);
	}
	...
	public boolean launch() {
		...
		for (Node node : shift.list()) {
			SeekSiteRuntime cmd = new SeekSiteRuntime(true); // 要求极速处理

 ...
 * @since laxcus 1.0
 */
public class RaySiteMemberBasket extends MutexHandler {

	/** 用户签名 -> 注册成员 **/
	private TreeMap<Siger, RayMemberMemo> sigers = new TreeMap<Siger, RayMemberMemo>();

	/** 节点地址 -> 节点成员 **/
	private TreeMap<Node, RaySiteMemo> sites = new TreeMap<Node, RaySiteMemo>();

	/**
	 * 构造注册成员合集
	 */
	protected RaySiteMemberBasket() {
		super();
	}
	...
	public boolean add(Seat seat) {
		...
		try {
			// 保存注册成员
			RayMemberMemo mm = sigers.get(seat.getSiger());
			if (mm == null) {
				mm = new RayMemberMemo(seat.getSiger());
				sigers.put(mm.getSiger(), mm);
			}
			boolean b1 = mm.add(seat.getSite());
			// 保存集群节点
			RaySiteMemo sm = sites.get(seat.getSite());
			if (sm == null) {
				sm = new RaySiteMemo(seat.getSite());
	...
	public boolean remove(Siger siger) {
		...
		super.lockSingle();
		try {
			RayMemberMemo mm = sigers.remove(siger);
			success = (mm != null);
			if (success) {
				for (Node e : mm.list()) {
					RaySiteMemo sm = sites.get(e);
					if (sm != null) {
						sm.remove(siger);
	...
	public boolean remove(Node node) {
		...
		super.lockSingle();
		try {
			RaySiteMemo sm = sites.get(node);
			success = (sm != null);
			if (success) {
				for (Siger e : sm.list()) {
					RayMemberMemo mm = sigers.get(e);
					if (mm != null) {
						mm.remove(node);
	...
	public List<Node> find(Siger siger) {
		...
		super.lockMulti();
		try {
			RayMemberMemo mm = sigers.get(siger);
			if (mm != null) {

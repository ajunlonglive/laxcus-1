 ...
 * @since laxcus 1.0
 */
public class RayInstallDialog extends LightDialog implements ActionListener, TreeSelectionListener {
	 ...
	 * @param e 安装工厂实例
	 */
	public RayInstallDialog(InstallFactory e) {
		super();
		factory = e;
		setRefreshUI(true);
	}
...
//	}
	...
	private File move(File file) {
		...
		// 移到应用目录
		File temp = new File(PlatformKit.getUserStoreRoot(), name);
	...
	private JScrollPane createTree() {
		...
		// 建立树型结构
		tree = new InstallTree(treeModel);
...
//		jsp.setBorder(new HighlightBorder(1));
	...
	private JSplitPane createHi() {
		...
		splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, createTree(), sub); // createTable());
		splitPane.setContinuousLayout(true);
		splitPane.setOneTouchExpandable(false);
		splitPane.setDividerSize(4); // 2像素

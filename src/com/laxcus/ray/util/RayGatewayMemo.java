 ...
 * @since laxcus 1.0
 */
public final class RayGatewayMemo implements Classable, Cloneable, Serializable , Comparable<RayGatewayMemo> {
	 ...
	 */
	private RayGatewayMemo(RayGatewayMemo that) {
		super();
		node = that.node;
	 ...
	 * @param node 节点地址
	 */
	public RayGatewayMemo(Node node) {
		this();
		setNode(node);
	}
	 ...
	 * @since 1.1
	 */
	public RayGatewayMemo(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 */
	public RayGatewayMemo duplicate() {
		return new RayGatewayMemo(this);
	}
	 ...
	 */
	@Override
	public int compareTo(RayGatewayMemo that) {
		if (that == null) {
			return 1;
		}

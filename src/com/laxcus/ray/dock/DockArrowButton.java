 ...
 * @since laxcus 1.0
 */
class DockArrowButton extends CraftButton {
...
//	private boolean pressed;
	 ...
	 * @param icon 图标
	 */
	public DockArrowButton(Icon icon) {
		super(icon);
		init();
	}
...
//		g.fillRect(0, 0, width, height);
		...
		if (icon != null) {
			...
			y += 2; // Y坐标下移2个像素
			if (x < 0) x = 0;
			else if(x >= width) x = width - iconWidth;
			if (y < 0) y = 0;
			else if (y >= height) y = height - iconHeight;
			// 绘制图标
			icon.paintIcon(this, g, x, y);
		}
...
//	}
	...
	protected void paintBorderX(Graphics g) {
		int width = getWidth();
		int height = getHeight();
		...
		if (isEnabled()) {
			...
			if (bm.isRollover()) {
				...
				paintFlatBorder(this, g, 0, 0, width, height);
			}
		} else {
			paintFlatBorder(this, g, 0, 0, width, height);
		}
	 ...
	 * @param height
	 */
	private void paintFlatBorder(Component c, Graphics g, int x, int y, int width, int height) {
		// 基于背景绘制边框

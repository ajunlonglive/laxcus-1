 ...
 * @since laxcus 1.0
 */
class DockBar extends RayPanel implements ActionListener, MouseListener {
	...
	/** 所属父类 **/
	private RayDock parent;

	/**
	 * 构造默认的应用坞面板
	 */
	public DockBar(RayDock d) {
		super();
		parent = d;
	}
	 ...
	 * @return
	 */
	private RayDock findDesktopDock(Component component) {
		Component parent = component.getParent();
		if (parent == null) {
			return null;
		}
		if (Laxkit.isClassFrom(parent, RayDock.class)) {
			return (RayDock) parent;
		}
	...
	void doMenuHideDocker() {
		RayDock dock = findDesktopDock(this);
		if (dock != null) {
	...
	private DockButton findButton(WKey key) {
		...
		for (DockButton but : array) {
			RayDockButtonItem item = but.getItem();
			if (item == null) {
				continue;
			}
	...
	private int doButtons(int barWidth) {
		// 可以显示的按纽数目
		return barWidth / (DockBanner.BUTTON_WIDTH + RayDock.DOCK_BUTTON_GAP);
	}
	...
	//			RayDockButtonItem item = button.getItem();
	//			if (item == null) {
	//				return null;
	//			}
	//			// 启动应用
	...
	public void writeUserButtons() {
		...
		try {
			int size = array.size();
			// 找到合适的对象
			ArrayList<RayDockButtonItem> a = new ArrayList<RayDockButtonItem>();
			for (int i = 0; i < size; i++) {
	 ...
	 * @return
	 */
	private RayDockButtonItem[] readUserElements() {
		File dir = RaySystem.createRuntimeDirectory();
		if (dir == null) {
			return new RayDockButtonItem[0];
		}
		// 配置目录下的字体文件
		File file = new File(dir, "docks.conf");
		// 没有这个文件，忽略它
		...
		ArrayList<RayDockButtonItem> array = new ArrayList<RayDockButtonItem>();
		try {
			...
			for(int i =0; i < size; i++){
				RayDockButtonItem item = new RayDockButtonItem(reader);
				array.add(item);
			}
		} catch (IOException e) {
			Logger.error(e);
		...
		// 输出
		int size = array.size();
		RayDockButtonItem[] a = new RayDockButtonItem[size];
	...
	private void showDefaultButtons() {
		// 读取本地的保存...
		RayDockButtonItem[] buttons = readUserElements();

		// 显示全部按纽
		for (RayDockButtonItem item : buttons) {
			addButton(item);
		}
	}

 ...
 * @since laxcus 1.0
 */
public class DataTalkAskInvoker extends DataInvoker {

	/**
	 * 构造分布任务组件远程交互调用器，指定命令
	 * @param cmd 分布任务组件远程交互
	 */
	public DataTalkAskInvoker(TalkAsk cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		if (invoker != null) {
			if (Laxkit.isClassFrom(invoker, DataConductFromInvoker.class)) {
				DataConductFromInvoker sub = (DataConductFromInvoker) invoker;
				reply = sub.ask(quest);
			} else if(Laxkit.isClassFrom(invoker, DataEstablishScanInvoker.class)) {
				DataEstablishScanInvoker sub = (DataEstablishScanInvoker) invoker;
				reply = sub.ask(quest);
			} else if(Laxkit.isClassFrom(invoker, DataEstablishRiseInvoker.class)) {
				DataEstablishRiseInvoker sub = (DataEstablishRiseInvoker) invoker;
				reply = sub.ask(quest);
			}
		}

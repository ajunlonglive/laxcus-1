 ...
 * @since laxcus 1.0
 */
public class DataPushArchiveSiteInvoker extends DataInvoker {

	/**
	 * 构造主动推送ARCHIVE站点命令调用器，指定命令
	 * @param cmd 主动推送ARCHIVE站点命令实例
	 */
	public DataPushArchiveSiteInvoker(PushAccountSite cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (Siger siger : cmd.list()) {
			// 签名有效
			boolean success = StaffOnDataPool.getInstance().allow(siger);
			// 保存账号和ARCHIVE站点地址
			if (success) {

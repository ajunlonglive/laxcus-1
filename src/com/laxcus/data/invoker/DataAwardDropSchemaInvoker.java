 ...
 * @since laxcus
 */
public class DataAwardDropSchemaInvoker extends DataInvoker {

	/**
	 * 构造授权删表命令调用器，指定命令
	 * @param cmd 授权删除表
	 */
	public DataAwardDropSchemaInvoker(AwardDropSchema cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 找到全部数据表
		List<Space> spaces = StaffOnDataPool.getInstance().findSpaces(fame);
		// 逐一删除
		for (Space space : spaces) {
			boolean success = StaffOnDataPool.getInstance().dropTable(space);

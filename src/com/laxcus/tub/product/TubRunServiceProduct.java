 ...
 * @since laxcus 1.0
 */
public class TubRunServiceProduct extends TubConfirmProduct {
	 ...
	 * 构造默认的检测边缘服务监听处理结果
	 */
	public TubRunServiceProduct() {
		super();
		setSuccessful(false);
	}
	
	public TubRunServiceProduct(boolean success) {
		this();
	 ...
	 * @param that
	 */
	private TubRunServiceProduct(TubRunServiceProduct that) {
		this();
		status = that.status;
	 ...
	 * @param reader 可类化读取器
	 */
	public TubRunServiceProduct(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 */
	@Override
	public TubRunServiceProduct duplicate() {
		return new TubRunServiceProduct(this);
	}

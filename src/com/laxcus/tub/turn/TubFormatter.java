 ...
 * @since laxcus 1.0
 */
public class TubFormatter implements Serializable {
	...
	protected Object readObject(ClassReader reader) {
		...
		switch (flag) {
		/** 空指针 **/
		case TubFormatter.NULL:
			return null;
		/** JAVA原生数据类型 **/
		case TubFormatter.TYPE_VOID:
			return java.lang.Void.TYPE;
		case TubFormatter.TYPE_BOOL:
			return java.lang.Boolean.TYPE;
		case TubFormatter.TYPE_BYTE:
			return java.lang.Byte.TYPE;
		case TubFormatter.TYPE_CHAR:
			return java.lang.Character.TYPE;
		case TubFormatter.TYPE_SHORT:
			return java.lang.Short.TYPE;
		case TubFormatter.TYPE_INT:
			return java.lang.Integer.TYPE;
		case TubFormatter.TYPE_LONG:
			return java.lang.Long.TYPE;
		case TubFormatter.TYPE_FLOAT:
			return java.lang.Float.TYPE;
		case TubFormatter.TYPE_DOUBLE:
			return java.lang.Double.TYPE;
		/** 各种数组 **/
		case TubFormatter.ARRAY_BYTE:
			return reader.readByteArray();
		case TubFormatter.ARRAY_BOOL:
			return reader.readBooleanArray();
		case TubFormatter.ARRAY_CHAR:
			return reader.readCharArray();
		case TubFormatter.ARRAY_SHORT:
			return reader.readShortArray();
		case TubFormatter.ARRAY_INT:
			return reader.readIntArray();
		case TubFormatter.ARRAY_LONG:
			return reader.readLongArray();
		case TubFormatter.ARRAY_FLOAT:
			return reader.readFloatArray();
		case TubFormatter.ARRAY_DOUBLE:
			return reader.readDoubleArray();
		case TubFormatter.ARRAY_OBJECT: {
			int size = reader.readInt();
			Object[] all = new Object[size];
			for (int index = 0; index < size; index++) {
				all[index] = readObject(reader);
	...
	protected byte[] writeObject(Object object) {
		...
		// 如果是空值，写入空标记，继续下一个
		if (object == null) {
			writer.write(TubFormatter.NULL);
		} else {
	...
	private byte[] writeArrayObject(Object that) {
		...
		if (clazz == byte[].class) {
			byte[] b = (byte[]) that;
			writer.write(TubFormatter.ARRAY_BYTE);
			writer.writeByteArray(b);
		} else if (clazz == boolean[].class) {
			boolean[] b = (boolean[]) that;
			writer.write(TubFormatter.ARRAY_BOOL);
			writer.writeBooleanArray(b);
		} else if (clazz == char[].class) {
			char[] b = (char[]) that;
			writer.write(TubFormatter.ARRAY_CHAR);
			writer.writeCharArray(b);
		} else if (clazz == short[].class) {
			short[] b = (short[]) that;
			writer.write(TubFormatter.ARRAY_SHORT);
			writer.writeShortArray(b);
		} else if (clazz == int[].class) {
			int[] b = (int[]) that;
			writer.write(TubFormatter.ARRAY_INT);
			writer.writeIntArray(b);
		} else if (clazz == long[].class) {
			long[] b = (long[]) that;
			writer.write(TubFormatter.ARRAY_LONG);
			writer.writeLongArray(b);
		} else if (clazz == float[].class) {
			float[] b = (float[]) that;
			writer.write(TubFormatter.ARRAY_FLOAT);
			writer.writeFloatArray(b);
		} else if (clazz == double[].class) {
			double[] b = (double[]) that;
			writer.write(TubFormatter.ARRAY_DOUBLE);
			writer.writeDoubleArray(b);
		} else if (that.getClass().isArray()) {
			byte[] b = writeAllObjects((Object[]) that);
			writer.write(TubFormatter.ARRAY_OBJECT);
			writer.write(b);
		} else {
			return null;
		}
	...
	private byte[] writePrimitiveType(Object object) {
		...
		if (object == java.lang.Void.TYPE) {
			writer.write(TubFormatter.TYPE_VOID);
		} else if (object == java.lang.Boolean.TYPE) {
			writer.write(TubFormatter.TYPE_BOOL);
		} else if (object == java.lang.Byte.TYPE) {
			writer.write(TubFormatter.TYPE_BYTE);
		} else if (object == java.lang.Character.TYPE) {
			writer.write(TubFormatter.TYPE_CHAR);
		} else if (object == java.lang.Short.TYPE) {
			writer.write(TubFormatter.TYPE_SHORT);
		} else if (object == java.lang.Integer.TYPE) {
			writer.write(TubFormatter.TYPE_INT);
		} else if (object == java.lang.Long.TYPE) {
			writer.write(TubFormatter.TYPE_LONG);
		} else if (object == java.lang.Float.TYPE) {
			writer.write(TubFormatter.TYPE_FLOAT);
		} else if (object == java.lang.Double.TYPE) {
			writer.write(TubFormatter.TYPE_DOUBLE);
		} else {
	...
	private byte[] writePrimitiveObject(Object object) {
		...
		if (clazz == Byte.class) {
			...
			writer.write(TubFormatter.PRIMITIVE_BYTE);
			writer.write(b);
		} else if (clazz == Boolean.class) {
			boolean b = ((Boolean) object).booleanValue();
			writer.write(TubFormatter.PRIMITIVE_BOOL);
			writer.writeBoolean(b);
		} else if (clazz == Character.class) {
			char ch = ((Character) object).charValue();
			writer.write(TubFormatter.PRIMITIVE_CHAR);
			writer.writeChar(ch);
		} else if (clazz == Short.class) {
			short value = ((Short) object).shortValue();
			writer.write(TubFormatter.PRIMITIVE_SHORT);
			writer.writeShort(value);
		} else if (clazz == Integer.class) {
			int value = ((Integer) object).intValue();
			writer.write(TubFormatter.PRIMITIVE_INT);
			writer.writeInt(value);
		} else if (clazz == Long.class) {
			long value = ((Long) object).longValue();
			writer.write(TubFormatter.PRIMITIVE_LONG);
			writer.writeLong(value);
		} else if (clazz == Float.class) {
			float value = ((Float) object).floatValue();
			writer.write(TubFormatter.PRIMITIVE_FLOAT);
			writer.writeFloat(value);
		} else if (clazz == Double.class) {
			double value = ((Double) object).doubleValue();
			writer.write(TubFormatter.PRIMITIVE_DOUBLE);
			writer.writeDouble(value);
		} else if (clazz == String.class) {
			String value = (String) object;
			writer.write(TubFormatter.PRIMITIVE_STRING);
			writer.writeString(value);
		} else if (clazz == Void.class) {
			writer.write(TubFormatter.PRIMITIVE_VOID);
		} else {
			return null;
		}
		return writer.effuse();
	...
	private byte[] writeClassableObject(Object object) {
		...
		writer.reset();
		// 写入可类化标记
		writer.write(TubFormatter.CLASSABLE);
		// 写入字节数组长度
	...
	private byte[] writeSerializableObject(Object object) {
		...
		ClassWriter writer = new ClassWriter();
		// 写入可类化标记
		writer.write(TubFormatter.SERIALIZABLE);
		// 写入字节数组长度
		writer.writeInt(b.length);
		// 写入字节数组
		writer.write(b);

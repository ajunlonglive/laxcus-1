 ...
 * @since laxcus 1.0
 */
public class MeetParallelMultiSwarmInvoker extends MeetInvoker {

	/**
	 * 构造并行流量测试调用器，指定命令
	 * @param cmd 并行流量测试命令
	 */
	public MeetParallelMultiSwarmInvoker(ParallelMultiSwarm cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (int i = 0; i < count; i++) {
			...
			multi.setSerial(i + 1);
			MeetMultiSwarmInvoker invoker = new MeetMultiSwarmInvoker(multi);

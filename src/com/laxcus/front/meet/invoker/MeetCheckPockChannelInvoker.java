 ...
 * @since laxcus 1.0
 */
public class MeetCheckPockChannelInvoker extends MeetInvoker {
	 ...
	 * @param cmd 应答包传输模式
	 */
	public MeetCheckPockChannelInvoker(CheckPockChannel cmd) {
		super(cmd);
	}
	...
	private void print(int family, SocketHost local, PockItem item) {
		...
		// 1. 类型
		if (family == MeetCheckPockChannelInvoker.CONTROL) {
		...
		} else if(family == MeetCheckPockChannelInvoker.SUCK){
			...
			showItem.add(new ShowStringCell(0, name, color));
		} else if(family == MeetCheckPockChannelInvoker.DISPATCH) {
	...
	//	 */
	//	private void checkDataPock() {
	//		java.util.List<PockItem> array = getLauncher().getReplyHelper()
	//		.getPocks();
	//		for (PockItem item : array) {
	//			print(false, item);
	//		}

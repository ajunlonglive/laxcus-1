 ...
 * @since laxcus 1.0
 */
public class DesktopShiftSwarmInvoker extends DesktopInvoker {

	/**
	 * 构造节点传输流量测试调用器，指定命令
	 * @param cmd 节点传输流量测试命令
	 */
	public DesktopShiftSwarmInvoker(ShiftSwarm cmd) {
		super(cmd);
	}
	 ...
	 */
	@Override
	public ShiftSwarm getCommand() {
		return (ShiftSwarm) super.getCommand();
	}
	...
	public boolean launch() {
		ShiftSwarm shift = getCommand();
		Swarm cmd = shift.getCommand();

		// FRONT站点必须使用默认的“HUB”关键字，指定向注册的GATE站点
		if (!cmd.isHub()) {
			shift.getHook().done();
			return false;
		}
	...
	public boolean ending() {
		...
		if (success) {
			getCommand().getHook().setResult(product);
		}
	 ...
	 * @return 传输成功返回真，否则假
	 */
	private TrafficProduct send(SwarmReflex reflux) {
	...
	private CastToken start(Cabin remote) {
		...
		Logger.debug(this, "start", token != null, "start to %s", remote);
	...
	private TrafficProduct upload(CastToken token) {
		Swarm cmd = getCommand().getCommand();
		...
		while (seek < end) {
			...
			if (!success) {
				Logger.error(this, "upload", "cannot send data, %s", client.getCastCode());
				break;
			}
			// 移到下标
			seek += len;
		...
		product.setSendSize(seek); // 数据发送尺寸

 ...
 * @since laxcus 1.0
 */
public class DesktopRenameCloudDirectoryInvoker extends DesktopHubServiceInvoker {

	/**
	 * 构造修改云端目录名称调用器
	 * @param cmd 修改云端目录名称
	 */
	public DesktopRenameCloudDirectoryInvoker(RenameCloudDirectory cmd) {
		super(cmd);
	}
	...
	public boolean ending() {
		RenameCloudDirectoryProduct product = null;
		int index = findEchoKey(0);
		try {
			if (isSuccessObjectable(index)) {
				product = getObject(RenameCloudDirectoryProduct.class, index);
			}
	...
	private void printResult(RenameCloudDirectoryProduct product) {
		// 显示处理结果
		printRuntime();
		// 显示标题
		createShowTitle(new String[] { "RENAME-CLOUD-DIRECTORY/STATE", "RENAME-CLOUD-DIRECTORY/DIRECTORY" });
		
		ShowItem item = new ShowItem();
		// 结果...
		int state = product.getState();
		if (StoreState.isFailed(state)) {
			String name = getXMLContent("RENAME-CLOUD-DIRECTORY/STATE/FAILED");
			item.add(new ShowStringCell(0, name));
		} else if (StoreState.isSuccessful(state)) {
			String name = getXMLContent("RENAME-CLOUD-DIRECTORY/STATE/SUCCESS");
			item.add(new ShowStringCell(0, name));
		} else if (StoreState.isExists(state)) {
			String name = getXMLContent("RENAME-CLOUD-DIRECTORY/STATE/EXISTS");
			item.add(new ShowStringCell(0, name));
		} else {
			String name = "None";
			item.add(new ShowStringCell(0, name));

 ...
 * @since laxcus 1.0
 */
public class DesktopAssertTableInvoker extends DesktopInvoker {

	/**
	 * 构造判断数据表调用器，指定命令
	 * @param cmd 判断数据表
	 */
	public DesktopAssertTableInvoker(AssertTable cmd) {
		super(cmd);
	}
	...
	private void print(boolean success) {
		...
		printRuntime();
		// 显示标题
		createShowTitle(new String[] { "ASSERT-TABLE/TABLE",
				"ASSERT-TABLE/STATUS" });

		// 结果
		String res = (success ? getXMLContent("ASSERT-TABLE/STATUS/YES")
				: getXMLContent("ASSERT-TABLE/STATUS/NO"));

		ShowItem item = new ShowItem();
		item.add(new ShowStringCell(0, space));

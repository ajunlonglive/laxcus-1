 ...
 * @since laxcus 1.0
 */
public class DesktopSystemDeployConductPackageInvoker extends DesktopSystemDeployCloudPackageInvoker {

	/**
	 * 构造部署分布计算应用软件包，指定命令
	 * @param cmd 部署分布计算应用软件包
	 */
	public DesktopSystemDeployConductPackageInvoker(DeployConductPackage cmd) {
		super(cmd);
	}
...
//		component.setSelfly(false);
	...
	private boolean deployTaskAssist(Naming ware, CloudPackageItem item) {
		...
		// 生成工作部件
		TaskPart part = new TaskPart(PhaseTag.PUT);
		part.setIssuer(null);
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.front.desktop.invoker.DesktopSystemDeployCloudPackageInvoker#deploySystem()
	 */
	@Override

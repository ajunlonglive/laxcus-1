 ...
 * @since laxcus 1.0
 */
public class DesktopSeekOnlineCommandInvoker extends DesktopInvoker {

	/**
	 * 构造检索在线命令调用器，指定命令
	 * @param cmd 检索在线命令
	 */
	public DesktopSeekOnlineCommandInvoker(SeekOnlineCommand cmd) {
		super(cmd);
	}
	
	/**
	 * SeekOnlineCommand命令在桌面是本地执行
	 */
	@Override
		...
		for (SeekOnlineCommandItem e : array) {
			...
			if (e.isSystem()) {
				...
				if (isSelfly(siger)) {
					...
					item.add(new ShowStringCell(2, siger));
				}
			}
			// 来源
			if (e.getSource() == null) {

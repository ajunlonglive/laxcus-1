 ...
 * @since laxcus 1.0
 */
public class DesktopDeployContactPackageInvoker extends DesktopDeployCloudPackageInvoker {

	/**
	 * 构造部署迭代计算应用软件包，指定命令
	 * @param cmd 部署迭代计算应用软件包
	 */
	public DesktopDeployContactPackageInvoker(DeployContactPackage cmd) {
		super(cmd);
	}
	...
	public boolean checkPermission() {
		...
		if (success) {
			DeployContactPackage cmd = this.getCommand();
			if (cmd.hasLibrary()) {
...
//		component.setSelfly(false);
	...
	protected PlayFruit deployLocal() {
		...
		// 读取包字节流
		byte[] stream = this.getPackageStream();
		if (stream == null) {

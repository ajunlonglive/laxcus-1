 ...
 * @since laxcus 1.0
 */
public abstract class DesktopShowReferenceInvoker extends DesktopInvoker {

	/**
	 * 构造显示数据资源的异步调用器，指定命令
	 * @param cmd 显示数据资源
	 */
	protected DesktopShowReferenceInvoker(Command cmd) {
		super(cmd);
	}
...
//		String ignore = (isDesktop() ? "| - " : "-");
		...
		// 备注
		String comment = (attribute.getComment() != null ? attribute.getComment() : "");
		item.add(new ShowStringCell(8, comment));

 ...
 * @since laxcus 1.0
 */
public class DesktopSystemDeployContactPackageInvoker extends DesktopSystemDeployCloudPackageInvoker {

	/**
	 * 构造部署迭代计算应用软件包，指定命令
	 * @param cmd 部署迭代计算应用软件包
	 */
	public DesktopSystemDeployContactPackageInvoker(DeployContactPackage cmd) {
		super(cmd);
	}
...
//		component.setSelfly(false);
	...
	private boolean deployTaskAssist(Naming ware, CloudPackageItem item) {
		...
		// 生成工作部件
		TaskPart part = new TaskPart(PhaseTag.NEAR);
		part.setIssuer(null);
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.front.desktop.invoker.DesktopSystemDeployCloudPackageInvoker#deployLocal()
	 */
	@Override

 ...
 * @since laxcus 1.0
 */
public class DesktopCheckRemoteTaskInvoker extends DesktopInvoker {

	/**
	 * 构造检测注册组件异步调用器，指定命令
	 * @param cmd 显示数据表命令
	 */
	public DesktopCheckRemoteTaskInvoker(CheckRemoteTask cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		CheckRemoteTaskProduct product = new CheckRemoteTaskProduct();
		...
		for (Sock sock : array) {
			// 找组件
			Phase phase = findPhase(sock);
			if (phase == null) {
				if (listener == null) {
					Object[] a = new Object[] { false, sock, "" };
					printRow(a);
				}
				continue;
			}

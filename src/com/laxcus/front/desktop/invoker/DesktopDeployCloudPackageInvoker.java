 ...
 * @since laxcus 1.0
 */
public abstract class DesktopDeployCloudPackageInvoker extends DesktopRuleInvoker {
	 ...
	 */
	protected DesktopDeployCloudPackageInvoker(DeployCloudPackage cmd) {
	...
	/**
	 * 读取包数据流
	 * @return 成功返回字节数组，否则是空指针
	 */
	protected byte[] getPackageStream() {
		DeployCloudPackage cmd = getCommand();
		byte[] stream = cmd.getContent();
		if (stream != null) {
			return stream;
		}
...
//	}
	...
	/*
	 * (non-Javadoc)
	 * @see com.laxcus.front.desktop.invoker.DesktopRuleInvoker#checkSubPermission()
	 */
	@Override
	protected boolean checkSubPermission() {
		// 如果来自分派接口时...
		ProductListener listener =	this.getProductListener();
		if(listener != null) {
			return true;
		}
...
//		File file = cmd.getFile();
		...
		String content = null;
		try {
			CloudPackageReader reader = new CloudPackageReader(stream);
...
//	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.laxcus.front.desktop.invoker.DesktopRuleInvoker#process()
	 */
	@Override
...
//	}
	...
	private String getSuffix() {
		DeployCloudPackage cmd = getCommand();
		if (cmd.getClass() == DeployConductPackage.class) {
	...
	private CloudPackageComponent createComponent() {
		...
		if (file != null) {
			...
			// 根据MD5的随机数
			String prefix = (stream == null ? "00000000000000000000": Laxkit.doMD5Hash(stream).toString());
			name = String.format("%s%s", prefix, getSuffix());
		}
		...
		// 返回结果
		return new CloudPackageComponent(name, stream);
	}

 ...
 * @since laxcus 1.0
 */
public class DesktopAssertUserInvoker extends DesktopInvoker {

	/**
	 * 构造判断用户名称调用器，指定命令
	 * @param cmd 判断用户名称
	 */
	public DesktopAssertUserInvoker(AssertUser cmd) {
		super(cmd);
	}
	...
	private void print(boolean success) {
		...
		printRuntime();
		// 显示标题
		createShowTitle(new String[] { "ASSERT-USER/USERNAME", "ASSERT-USER/STATUS" });

		// 结果
		String res = (success ? getXMLContent("ASSERT-USER/STATUS/YES") : getXMLContent("ASSERT-USER/STATUS/NO"));

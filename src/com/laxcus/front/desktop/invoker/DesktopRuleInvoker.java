 ...
 * @since laxcus 1.0
 */
public abstract class DesktopRuleInvoker extends DesktopInvoker {
	 ...
	 */
	protected DesktopRuleInvoker(Command cmd, boolean lockScheduleRefresh) {
		super(cmd);
	 ...
	 * @param cmd 命令
	 */
	protected DesktopRuleInvoker(Command cmd) {
		this(cmd, false);
	}
	...
	private boolean insure() {
		...
		if (success) {
			...
			if (!hasProductListener()) {
				messageX(MessageTip.RULE_ATTACH, getInvokerId());
			}
		} else {
	...
	private boolean checkInsure() {
		...
		if (success) {
			// 打印信息，接受事务处理
			if (!hasProductListener()) {
				messageX(MessageTip.RULE_ATTACH_RETURN_OK, getInvokerId());
			}
		} else {
	...
	private boolean revoke() {
		...
		if (success) {
			if (!hasProductListener()) {
				messageX(MessageTip.RULE_DETACH, getInvokerId());
			}
		} else {
	...
	private boolean checkRevoke() {
		...
		if (success) {
			if (!hasProductListener()) {
				messageX(MessageTip.RULE_DETACH_RETURN_OK, getInvokerId());
			}
		} else {

 ...
 * @since laxcus 1.0
 */
public class DesktopMultiSwarmInvoker extends DesktopInvoker {

	/**
	 * 构造多节点流量测试调用器，指定命令
	 * @param cmd 多节点流量测试命令
	 */
	public DesktopMultiSwarmInvoker(MultiSwarm cmd) {
		super(cmd);
	}
	 ...
	 */
	@Override
	public MultiSwarm getCommand() {
		return (MultiSwarm) super.getCommand();
	}
	...
	public boolean launch() {
		...
		Swarm sub = cmd.getSwarm();

		int faults = 0;
		for (int i = 0; i < count; i++) {
			SwarmHook hook = new SwarmHook();
			ShiftSwarm shift = new ShiftSwarm(sub, hook);
			shift.setIssuer(getIssuer());

			boolean success = press(shift, getDisplay());
			if (!success) {
				faults++;
				continue;
			}
		...
		if (listener != null) {
			MultiSwarmProduct sp = new MultiSwarmProduct(faults, array);
			listener.push(sp);
		} else {
			// 显示结果
			print(faults, array);
	...
	private void print(int faults, List<TrafficProduct> array) {
		...
		// 单个任务的资源消耗时间. 总时间 - 流量时间 / 进程数
		long sourceTime = allTime - runtime;
		long singaleTaskResourceTime = sourceTime / (faults + array.size());
		...
		// 全部任务资源耗时
		value = doStyleTime(sourceTime);
		show("MULTI-TRAFFIC/SOURCE-TIME", new ShowStringCell(1, value));

		// 单个任务资源耗时
		value = doStyleTime(singaleTaskResourceTime);
		show("MULTI-TRAFFIC/SINGLE-TASK-TIME", new ShowStringCell(1, value));

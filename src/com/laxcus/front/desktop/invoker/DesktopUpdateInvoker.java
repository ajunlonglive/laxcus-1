 ...
 * @since laxcus 1.0
 */
public class DesktopUpdateInvoker extends DesktopQueryInvoker {

	/**
	 * 构造UPDATE命令调用器，指定UPDATE命令
	 * @param cmd - UPDATE命令
	 */
	public DesktopUpdateInvoker(Update cmd) {
		super(cmd);
	}
	...
	private boolean createDirectUpdate() {
		Update update = getCommand();
		DesktopDirectUpdateInvoker invoker = new DesktopDirectUpdateInvoker(update);

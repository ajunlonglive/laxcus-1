 ...
 * @since laxcus 1.0
 */
public class DesktopNotifyDialog extends LightDialog implements MeetDisplay {
	...
	/** 固定的窗口句柄 **/
	private static DesktopNotifyDialog selfHandle;

	/**
	 * 返回实例句柄
	 * @return 句柄
	 */
	public static DesktopNotifyDialog getInstance(JDesktopPane desktop) {
		if (desktop == null) {
			throw new NullPointerException("DesktopPane is null!");
		}
		// 没有定义，构造函数
		if (DesktopNotifyDialog.selfHandle == null) {
			DesktopNotifyDialog.selfHandle = new DesktopNotifyDialog(desktop);
		}
		return DesktopNotifyDialog.selfHandle;
	}
	 ...
	 * @return 实例
	 */
	public static DesktopNotifyDialog getInstance() {
		if (DesktopNotifyDialog.selfHandle == null) {
			throw new NullPointerException("DesktopPane is null!");
		}
		return DesktopNotifyDialog.selfHandle;
...
//	private JMenuBar menubar;
	 ...
	 * 构造默认的提示对话框
	 */
	private DesktopNotifyDialog(JDesktopPane d) {
		super();
		desktop = d;
//		// 使用固定的UI界面
//		setFixedNimbusBorder(true);
	...
	public void closeWindow() {
		...
		setLoaded(false);
		// 清除...
		DesktopNotifyDialog.selfHandle = null;
...
//	}
	...
	public void doShow() {
		...
		if (!isLoaded()) {
			// 打开窗口
			DesktopNotifyDialog.selfHandle.showDialog(desktop);
		}
		// 如果窗口加载，这时要切换到最前面
		else {
			JInternalFrame[] frames = desktop.getAllFrames();

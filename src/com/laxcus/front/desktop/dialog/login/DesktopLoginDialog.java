 ...
 * @since laxcus 1.0
 */
public class DesktopLoginDialog extends CommonFontDialog implements ActionListener {
...
//	private JTextField txtUser = new JTextField();
	 ...
	 * 构造图形前端登录窗口
	 * @param frame 主窗口
	 * @param ResetHandler 重置为初始状态
	 * @throws HeadlessException
	 */
	public DesktopLoginDialog(Frame frame, ResetHandler rh) {
		super(frame);
		handler = rh;
	...
	private void loadLicence() {
		...
		if (words != null && words.length > 0) {
			...
			DesktopLauncher.getInstance().setSignature(signature);
			DesktopLauncher.getInstance().loadLicence(false);
		}
	}
	 ...
	 */
	private void login() {
		DesktopLoginThread thread = new DesktopLoginThread(this);
	...
	private void showXMLFailed(String xmlMessage, String xmlTitle) {
		String title = UIManager.getString(xmlTitle);
		String content = UIManager.getString(xmlMessage);
	...
	void showLoginFailed(String host, String port, int who, int pitchId, SiteHost pitchHub) {
		String title = UIManager.getString("LoginDialog.Error.ConnectFiledTitle");
		String content = "";

		if (who == FrontEntryFlag.ENTRANCE_FAULT) {
			content = UIManager.getString("LoginDialog.Error.EntranceFailedText");
		} else if (who == FrontEntryFlag.CANNOT_REDIRECT) {
			content = UIManager.getString("LoginDialog.Error.RedirectFailedText");
		} else if (who == FrontEntryFlag.GATE_FAULT) {
			content = UIManager.getString("LoginDialog.Error.GateFailedText");
		} else if(who == FrontEntryFlag.NAT_FAULT) {
			content = UIManager.getString("LoginDialog.Error.NatFailedText");
		} else if(who == FrontEntryFlag.MAX_USER) {
			content = UIManager.getString("LoginDialog.Error.MaxUserText");
		} else if(who == FrontEntryFlag.LOGIN_TIMEOUT) {
			content = UIManager.getString("LoginDialog.Error.LoginTimeoutText");
		} else if(who == FrontEntryFlag.SERVICE_MISSING) {
			content = UIManager.getString("LoginDialog.Error.ServiceMissingText");
		} else if(who == FrontEntryFlag.REFLECT_FAULT) {
			showPitchFailed(pitchId, pitchHub);
			return;
		} else if(who == FrontEntryFlag.MAX_RETRY) {
			content = UIManager.getString("LoginDialog.Error.MaxRetryText");
		} else if(who == FrontEntryFlag.LICENCE_NAT_REFUSE) {
			content = UIManager.getString("LoginDialog.Error.LicenceNatRefuseText");
			showWarning( title, content);
			return;
		} else if (who == FrontEntryFlag.VERSION_NOTMATCH) {
			content = UIManager.getString("LoginDialog.Error.VersionNotmatchText");
		}
		// 其它错误
		else {
			content = UIManager.getString("LoginDialog.Error.ConnectFailedText");
	...
	private void showPitchFailed(int pitchId, SiteHost pitchHub) {
		if (pitchId == FrontPitch.NOT_FOUND) {
			...
			String title = UIManager.getString("LoginDialog.Error.Pitch.NoutFoundTitle");
			String content = UIManager.getString("LoginDialog.Error.Pitch.NoutFoundText");
			content = String.format(content, host);
			showFailed(title, content);
		} else {
			String title = UIManager.getString("LoginDialog.Error.ConnectFiledTitle");
			String content = UIManager.getString("LoginDialog.Error.ReflectFailedText");
			showFailed(title, content);
		}
	}
	 ...
	 * @return 成功返回“真”，失败“假”。
	 */
	private boolean check(DesktopLoginThread thread) {
	...
	private void createControls() {
		...
		cmdLicence.setRolloverEnabled(true);
		...
		cmdReset.setRolloverEnabled(true);

		// 取出系统中的签名，显示它!
		String signature = DesktopLauncher.getInstance().getSignature();
		if (signature != null) {
	...
	private JPanel createIconPanel() {
		Icon icon = UIManager.getIcon("LoginDialog.UserIcon");
...
//		System.out.printf("password ui is %s\n", txtPwd.getUI().getClass().getName());
	...
	public void showDialog(LoginToken token) {
		...
		// 关闭启动屏幕窗口
		DesktopLauncher.getInstance().stopSplash();

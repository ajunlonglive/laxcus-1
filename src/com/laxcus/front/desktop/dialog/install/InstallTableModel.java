 ...
 * @since laxcus 1.0
 */
class InstallTableModel extends DefaultTableModel {

	private static final long serialVersionUID = -3751251076697750030L;
	...
	public String getColumnName(int column) {
		if (title == null || column < 0 || column >= title.size()) {
			return null;
		}
...
//		if (Laxkit.isClassFrom(value, String.class)) {
//			return;
//		} else if (Laxkit.isClassFrom(value, Boolean.class)) {
//			return;
//		}
		
		// 如果是部署桌面图标...
		if (Laxkit.isClassFrom(value, InstallDesktopIcon.class)) {
			InstallDesktopIcon di = (InstallDesktopIcon) value;
			ShowItemCell cell = getCellAt(row, column);
			if (cell != null && Laxkit.isClassFrom(cell, ShowBooleanCell.class)) {
				ShowBooleanCell c = (ShowBooleanCell) cell;
				c.setValue(di.isDeploy());
			}
			return;
		}
		// 如果是部署应用坞图标...
		if (Laxkit.isClassFrom(value, InstallDockIcon.class)) {
			InstallDockIcon di = (InstallDockIcon) value;
			ShowItemCell cell = getCellAt(row, column);
			if (cell != null && Laxkit.isClassFrom(cell, ShowBooleanCell.class)) {
				ShowBooleanCell c = (ShowBooleanCell) cell;
				c.setValue(di.isDeploy());
		...
		if (!Laxkit.isClassFrom(value, ShowItemCell.class)) {
			throw new ClassCastException("cannot be cast to ShowItemCell! " + value.getClass().getName());
		}

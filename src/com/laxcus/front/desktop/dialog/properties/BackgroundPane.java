 ...
 * @since laxcus 1.0
 */
class BackgroundPane extends DesktopPanel implements ActionListener {
...
//			choiceCloudFile();
	...
	private void saveBackground() {
		...
		if (color != null) {
			RTKit.writeColor(RTEnvironment.ENVIRONMENT_SYSTEM, "DesktopWindow/Background/Color", color);
		} else {
			RTKit.remove(RTEnvironment.ENVIRONMENT_SYSTEM, "DesktopWindow/Background/Color",
		...
		if (file != null) {
			...
			RTKit.writeInteger(RTEnvironment.ENVIRONMENT_SYSTEM, "DesktopWindow/Background/Layout", wall.getLayout());
		} else {
			RTKit.remove(RTEnvironment.ENVIRONMENT_SYSTEM, "DesktopWindow/Background/File", RTokenAttribute.PARAMETER);
			RTKit.remove(RTEnvironment.ENVIRONMENT_SYSTEM, "DesktopWindow/Background/Layout", RTokenAttribute.PARAMETER);
		}
	}
...
//			startTime = System.currentTimeMillis();
		...
		public void process() {
			// 空指针时，忽略
			if (DesktopPropertiesDialog.selfHandle == null) {
				return;
			}
...
//		label.setText(UIManager.getString("PropertiesDialog.backgroundIntroduceText"));
	...
	private JPanel createLayoutPanel() {
		imgModel.addElement(new BackgroundImageItem(null, UIManager.getString("PropertiesDialog.backgroundImageListNone")));
...
//		boxLayout.setEditor(new FlatComboBoxEditor());
//		// 处理它
//		ComboBoxEditor cp =	boxLayout.getEditor();
//		System.out.printf("combobox editor is %s\n", cp.getClass().getName());
//		System.out.printf("combobox editor component is %s\n", cp.getEditorComponent().getClass().getName());
	}

 ...
 * @since laxcus 1.0
 */
public class DesktopDock extends LightForm {
...
//		setBorder(new EmptyBorder(1,1,1,1));
	...
	public int readLayer() {
		...
		// 读一个整数，默认是POPUP_LAYER，有窗口前面
//		return RTKit.readInteger(RTEnvironment.ENVIRONMENT_SYSTEM, key, JLayeredPane.POPUP_LAYER.intValue());
...
//		int layer = (who == JLayeredPane.POPUP_LAYER ? JLayeredPane.DEFAULT_LAYER : JLayeredPane.POPUP_LAYER);
...
//			int dockWidth = getPreferredWidth();
	 ...
	 * 显示窗口
	 */
	public void showWindow(DesktopEdgeListener listener) {
		setScreenBounds();
		...
		// 图标和标题
		//		setFrameIcon(findImage("conf/desktop/image/frame/log.png", 16, 16));
...
//	}
	...
	public void doMoveToTop() {
		...
		r = new Rectangle(x, 0, r.width, DesktopDock.DOCK_HEIGHT);
		setBounds(r);
	}
	...
	public void doMoveToTopFull() {
		...
		Dimension d = desktop.getSize();
		Rectangle r = getBounds();
		r = new Rectangle(0, 0, d.width, DesktopDock.DOCK_HEIGHT);
		setBounds(r);
		// 调整宽度
		modifyWidth(true);
	}
	...
	public void doMoveMiddle() {
		...
		int x = (d.width - r.width) / 2;
		// 重置
		r = new Rectangle(x, r.y, r.width, DesktopDock.DOCK_HEIGHT);
		setBounds(r);
	}
	...
	public void doMoveToBottom() {
		...
		// 重置
		r = new Rectangle(x, y, r.width, DesktopDock.DOCK_HEIGHT);
		setBounds(r);
	}
	...
	public void doMoveToBottomFull() {
		...
		int y = d.height - r.height;
		// 重置
		r = new Rectangle(0, y, d.width, DesktopDock.DOCK_HEIGHT);
		setBounds(r);
		// 重置联署
		modifyWidth(true);
	}

 ...
 * @since laxcus 1.0
 */
public class DesktopLaunchMenuCreator {
...
//		}
	...
	/**
	 * 根据方法名称，查找菜单项
	 * @param desktopLaunchMenu 弹出菜单
	 * @param method 方法名称
	 * @return 返回匹配方法名称的菜单项目，没有是空指针
	 */
	public JMenuItem findMenuItemByMethod(DesktopLaunchMenu desktopLaunchMenu, String method) {
		int count = desktopLaunchMenu.getComponentCount();
		for (int index = 0; index < count; index++) {
			Component element = desktopLaunchMenu.getComponent(index);
			if (element == null) {
				continue;
			}
			// 判断是JMenu
...
//	}
	 ...
	 * @return
	 */
	public static JMenu findMenu(DesktopLaunchMenu root, String associate) {
		if (associate == null) {
			return null;
		}
		// 找到...
...
//	}
	...
	private String getId(Element root) {
		return getAttribute(root, DesktopLaunchMenuCreator.ID);
	}
	...
	private String getHotKey(Element root) {
		return getAttribute(root, DesktopLaunchMenuCreator.HOTKEY);
	}
	...
	private String getAssociate(Element root) {
		return getAttribute(root, DesktopLaunchMenuCreator.ASSOCIATE);
	}
...
//	}
	...
	private boolean isCheckOn(Element root) {
		String value = getAttribute(root, DesktopLaunchMenuCreator.CHECKON);
		if (value != null) {
	...
	private boolean isRadioOn(Element root) {
		String value = getAttribute(root, DesktopLaunchMenuCreator.RADIOON);
		if (value != null) {
			return value.matches("^\\s*(?i)(TRUE|YES|ON)\\s*$");
		}
		return false;
...
//			menu.setName(method.trim());
	 ...
	 */
	public DesktopLaunchMenu create(byte[] xml) {
		...
		DesktopLaunchMenu desktopLaunchMenu = new DesktopLaunchMenu();
		...
		for (int i = 0; i < size; i++) {
			...
			if (isFlag(node)) {
				...
				if (text != null) {
					desktopLaunchMenu.setFlagText(text.trim());
				}
			}
			// 三种菜单项
			else if (isMenuItem(node)) {
				JMenuItem sub = createMenuItem(new JMenuItem(), (Element) node);
				desktopLaunchMenu.add(sub);
			} else if (isCheckBoxMenuItem(node)) {
				JMenuItem sub = createMenuItem(new JCheckBoxMenuItem(), (Element) node);
				desktopLaunchMenu.add(sub);
			} else if (isRadioButtonMenuItem(node)) {
			...
			// 分隔符
				...
				desktopLaunchMenu.addSeparator(); // 增加一个分隔栏
			}
		}
		
		// 返回菜单条
		return desktopLaunchMenu;
	}
	 ...
	 */
	public DesktopLaunchMenu create(String name) {
	...
	public static void main(String[] args) {
		...
		DesktopLaunchMenuCreator e = new DesktopLaunchMenuCreator();
		DesktopLaunchMenu bar = e.create(xml);

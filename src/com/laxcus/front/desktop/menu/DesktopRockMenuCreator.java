 ...
 * @since laxcus 1.0
 */
public class DesktopRockMenuCreator {
	 ...
	 * @return
	 */
	public static JMenu findMenu(DesktopRockMenu root, String associate) {
	 ...
	 */
	public static JMenuItem findMenuItemByMethod(DesktopRockMenu root, String method) {
		int count = root.getComponentCount();
		for (int index = 0; index < count; index++) {
			Component element = root.getComponent(index);
			if (element == null) {
...
//	}
	...
	private String getId(Element root) {
		return getAttribute(root, DesktopRockMenuCreator.ID);
	}
	...
	private String getHotKey(Element root) {
		return getAttribute(root, DesktopRockMenuCreator.HOTKEY);
	}
	...
	private String getAssociate(Element root) {
		return getAttribute(root, DesktopRockMenuCreator.ASSOCIATE);
	}
	...
	private boolean isCheckOn(Element root) {
		String value = getAttribute(root, DesktopRockMenuCreator.CHECKON);
		if (value != null) {
	...
	private boolean isRadioOn(Element root) {
		String value = getAttribute(root, DesktopRockMenuCreator.RADIOON);
		if (value != null) {
			return value.matches("^\\s*(?i)(TRUE|YES|ON)\\s*$");
		}
		return false;
...
//		}
	 ...
	 * @return
	 */
	public DesktopRockMenu create(Element root) {
		NodeList nodes = root.getChildNodes();
		int size = nodes.getLength();
		
		DesktopRockMenu desktopRockMenu = new DesktopRockMenu();
		...
		for (int i = 0; i < size; i++) {
			...
			// 分隔符
				...
				desktopRockMenu.addSeparator(); // 增加一个分隔栏
			}
		}
		
		return desktopRockMenu;
	}
	 ...
	 * @return 返回弹出菜单
	 */
	public DesktopRockMenu create(String name, String xmlRoot) {
		// 解析

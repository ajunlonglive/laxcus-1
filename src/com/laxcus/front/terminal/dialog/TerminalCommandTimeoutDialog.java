 ...
 * @since laxcus 1.0
 */
public class TerminalCommandTimeoutDialog extends TerminalCommonFontDialog implements ActionListener {

	private static final long serialVersionUID = 1742930552683548144L;
	 ...
	 */
	public TerminalCommandTimeoutDialog(Frame parent, boolean modal) {
		super(parent, modal);
	}
	...
	private void active(ActionEvent event) {
		if (event.getSource() == cmdOK) {
			...
			if (interval < 12000) {
				// 提示错误
				String title = findCaption("Dialog/CommandTimeout/warning/title");
				String content = findContent("Dialog/CommandTimeout/warning");
				MessageDialog.showMessageBox(this, title, JOptionPane.WARNING_MESSAGE, content, JOptionPane.DEFAULT_OPTION);
				return;
			}

			CommandTimeout cmd = new CommandTimeout(interval);
...
//	}
	...
	private void initControls() {
		...
		// 间隔时间
		long interval = TerminalLauncher.getInstance().getCommandTimeout();
		// 取出时间下标
	...
	private JPanel createComboxPanel() {
		...
		setLabelText(hour, findCaption("Dialog/CommandTimeout/time/hour/title"));
		hour.setDisplayedMnemonic('H');
		hour.setLabelFor(boxHour);

		JLabel minute = new JLabel("", SwingConstants.LEFT);
		setLabelText(minute, findCaption("Dialog/CommandTimeout/time/minute/title"));
		minute.setDisplayedMnemonic('M');
		minute.setLabelFor(boxMinute);

		JLabel second = new JLabel("", SwingConstants.LEFT);
		setLabelText(second, findCaption("Dialog/CommandTimeout/time/second/title"));
		...
		String title = findCaption("Dialog/CommandTimeout/time/title");
...
//				TitledBorder.ABOVE_TOP, font));
	...
	private JPanel createButtonPanel() {
		setButtonText(cmdOK, findCaption("Dialog/CommandTimeout/buttons/okay/title"));
		cmdOK.setMnemonic('O');
		cmdOK.addActionListener(this);

		setButtonText(cmdCancel, findCaption("Dialog/CommandTimeout/buttons/cancel/title"));
...
//		canvas.add(pane, BorderLayout.CENTER);
		...
		// 标题
		String title = findCaption("Dialog/CommandTimeout/title");
		setTitle(title);

 ...
 * @since laxcus 1.0
 */
public class TerminalCreateEstablishScriptDialog extends TerminalCreateWareScriptDialog implements ActionListener {

	private static final long serialVersionUID = -2001226139409618699L;

	protected final static String BOUND = TerminalCreateEstablishScriptDialog.class.getSimpleName() + "_BOUND";
	
	/** 窗口任务实体 **/
	
	private TaskBody issue = new TaskBody(PhaseTag.ISSUE);

	private TaskBody scan = new TaskBody(PhaseTag.SCAN);
	
	private TaskBody assign = new TaskBody(PhaseTag.ASSIGN);

	private TaskBody sift = new TaskBody(PhaseTag.SIFT);
	
	private TaskBody rise = new TaskBody(PhaseTag.RISE);

	private TaskBody end = new TaskBody(PhaseTag.END);
	...
	private TaskBody[] getTasks() {
		return new TaskBody[] { issue, scan, assign, sift, rise, end };
	}
	 ...
	 * @param modal
	 */
	public TerminalCreateEstablishScriptDialog(Frame owner, boolean modal) {
		super(owner, modal);
	}
	...
	private void saveBound() {
		...
		if (e != null) {
			UITools.putProperity(TerminalCreateEstablishScriptDialog.BOUND, e);
		}
	}
...
//		}
	...
	private Rectangle getBound() {
		// 面板范围
		Object obj = UITools.getProperity(TerminalCreateEstablishScriptDialog.BOUND);
	...
	public void showDialog() {
		...
		// 标题
		String title = findContent("Dialog/CreateWareScript/title/establish");
		setTitle(title);

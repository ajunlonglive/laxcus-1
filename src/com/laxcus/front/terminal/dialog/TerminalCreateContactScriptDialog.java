 ...
 * @since laxcus 1.0
 */
public class TerminalCreateContactScriptDialog extends TerminalCreateWareScriptDialog implements ActionListener {

	private static final long serialVersionUID = 5367302604928746848L;

	protected final static String BOUND = TerminalCreateContactScriptDialog.class.getSimpleName() + "_BOUND";
	
	/** 窗口任务实体 **/
	private TaskBody fork = new TaskBody(PhaseTag.FORK);

	private TaskBody merge = new TaskBody(PhaseTag.MERGE);

	private TaskBody distant = new TaskBody(PhaseTag.DISTANT);

	private TaskBody near = new TaskBody(PhaseTag.NEAR);
	...
	private TaskBody[] getTasks() {
		return new TaskBody[] { fork, merge, distant, near };
	}
	 ...
	 * @param modal
	 */
	public TerminalCreateContactScriptDialog(Frame owner, boolean modal) {
		super(owner, modal);
	}
	...
	private void saveBound() {
		...
		if (e != null) {
			UITools.putProperity(TerminalCreateContactScriptDialog.BOUND, e);
		}
	}
...
//		}
	...
	private Rectangle getBound() {
		// 面板范围
		Object obj = UITools.getProperity(TerminalCreateContactScriptDialog.BOUND);
	...
	public void showDialog() {
		...
		// 标题
		String title = findContent("Dialog/CreateWareScript/title/contact");
		setTitle(title);

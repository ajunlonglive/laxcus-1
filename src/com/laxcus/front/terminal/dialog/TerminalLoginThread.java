 ...
 * @since laxcus 1.0
 */
class TerminalLoginThread implements Runnable {

	/** 窗口句柄 **/
	private TerminalLoginDialog dialog;
	 ...
	 * @param e
	 */
	public TerminalLoginThread(TerminalLoginDialog e) {
		super();
		dialog = e;
	}
	...
	public void run() {
		// 分配追踪器句柄
		TerminalTrackDialog tracker = new TerminalTrackDialog(dialog, true);
		...
		// 设置用户名和密码
		FrontSite local = TerminalLauncher.getInstance().getSite();
		...
		// 启动登录
		int who = TerminalLauncher.getInstance().login(hub, false, tracker);

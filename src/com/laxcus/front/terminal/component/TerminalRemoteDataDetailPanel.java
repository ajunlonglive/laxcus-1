 ...
 * @since laxcus 1.0
 */
public class TerminalRemoteDataDetailPanel extends JPanel {

	private static final long serialVersionUID = -1000171773451349053L;

	private TerminalRemoteDataPanel parent;

	/** 渲染器 **/
	private TerminalRemoteDataDetailCellRenderer renderer;

	/** 表格模型 **/
	private TerminalRemoteDataDetailModel tableModel;
...
//	private String tagNotFound;  // 没有找到
	 ...
	 * 构造用户注册/登录游览窗口
	 */
	public TerminalRemoteDataDetailPanel() {
		super();
	}
...
//		tableModel.addRow(createItem(tagCloudTask, phase.toString()));
//		
//		// 服务器地址参数
...
//		}
//		if (exists) {
//			tableModel.addRow(createItem(tagLocalTask, sub.toString()));
//		} else {
...
//		} 
	...
	private void initHeader() {
		// 属性/参数行的宽度
		String value = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/key-width");
		int key_width = ConfigParser.splitInteger(value, 42);
		value =  TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/value-width");
		int value_width = ConfigParser.splitInteger(value, 130);

		String keyText =  TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Header/Key/title");
		String valueText =  TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Header/Value/title");
		...
		//			columns[i].setHeaderRenderer(new TerminalRemoteBrowserDetailHeaderCellRenderer(name));
	...
	public void init() {
		//  取配置参数
		String titleText = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/title");
		FontKit.setToolTipText(this, titleText);

		// 行高度
		String value = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/row-height");
		int rowHeight = ConfigParser.splitInteger(value, 30);
		// 表头高度
		value = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/header-height");
		int headerHeight = ConfigParser.splitInteger(value, 28);

		// 表模型
		tableModel = new TerminalRemoteDataDetailModel();
		table = new JTable(tableModel);
		// 渲染器只支持ShowItemCell类
		renderer = new TerminalRemoteDataDetailCellRenderer();
		...
		//		// 所有支持OBJECT类都使用这个渲染器
		//		renderer = new TerminalRemoteBrowserDetailRowCellRenderer();
		...
		// 属性名
		tagAuthorizer = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Authorizer/title");
		tagAuthorizerOperator = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/AuthorizerOperator/title");

		tagTable = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Table/title");
		tagCreateTime = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/CreateTime/title");

//		tagCloudTask = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/CloudTask/title");
//		tagLocalTask = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/LocalTask/title");
//		tagNotFound = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/NotFound/title");
		
		tagPosition = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Position/title");
	...
	//		int headerHeight = ConfigParser.splitInteger(value, 28);
	//		// 属性/参数行的宽度
	//		value = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/key-width");
	//		int key_width = ConfigParser.splitInteger(value, 42);
	//		value = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/value-width");
	//		int value_width = ConfigParser.splitInteger(value, 130);
	//		
	...
	//
	//		// 所有支持OBJECT类都使用这个渲染器
	//		table.setDefaultRenderer(Object.class, new TerminalRemoteBrowserDetailRowCellRenderer());
	//		
	//		// 设置表头的最优范围
	...
	//		
	//		// 信息
	//		sigerText = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Siger/title");
	//		registerText =	TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Register/title");
	//		register_onlineText = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Register-Online/title");
	//		onlineText = TerminalLauncher.getInstance().findCaption("Window/RemoteDataDetailPanel/Online/title");
	//
	//		// 定位复制键
	//		table.getInputMap(JComponent.WHEN_FOCUSED).put(
	//				KeyStroke.getKeyStroke(KeyEvent.VK_C, KeyEvent.CTRL_DOWN_MASK, true), "CTRL C");
	...
	//		
	//		// 读取定义字体，更新！
	//		Font font = TerminalProperties.readBrowserRemoteFont();
	//		if (font != null) {
	//			setSelectFont(font);
	//		}
	//	}

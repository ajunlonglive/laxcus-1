 ...
 * 基于边缘容器模式的事务调用器 <br>
 * 
 * 边缘容器事务调用器提供了默认的数据规则处理流程，子类将按照这个规则执行处理。
 * 
 * @author scott.liang
 * @version 1.23 09/06/2016
 * @since laxcus 1.0
 */
public abstract class TubRuleInvoker extends TubInvoker {
	...
	/**
	 * 构造基于边缘容器模式的事务调用器，指定边缘容器任务
	 * @param mission 边缘容器任务
	 */
	protected TubRuleInvoker(TubMission mission) {
		super(mission);
	}

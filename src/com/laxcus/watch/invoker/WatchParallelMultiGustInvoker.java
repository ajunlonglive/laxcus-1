 ...
 * @since laxcus 1.0
 */
public class WatchParallelMultiGustInvoker extends WatchInvoker {

	/**
	 * 构造并行流量测试调用器，指定命令
	 * @param cmd 并行流量测试命令
	 */
	public WatchParallelMultiGustInvoker(ParallelMultiGust cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (int i = 0; i < count; i++) {
			...
			real.setSerial(i + 1);
			WatchMultiGustInvoker invoker = new WatchMultiGustInvoker(real);

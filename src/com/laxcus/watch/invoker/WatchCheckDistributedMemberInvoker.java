 ...
 * @since laxcus 1.0
 */
public class WatchCheckDistributedMemberInvoker extends WatchInvoker {

	/**
	 * 构造检查集群成员分布调用器，指定命令
	 * @param cmd 检查集群成员分布
	 */
	public WatchCheckDistributedMemberInvoker(CheckDistributedMember cmd) {
		super(cmd);
	}
	...
	private void print(CheckDistributedMember cmd) {
		...
		// 显示全部！
		if (cmd.isAll()) {
			sigers = RegisterMemberBasket.getInstance().getUsers();
		}
		...
		for (int index = 0; index < size; index++) {
			...
			if (username != null && Siger.validate(username)) {
				boolean success = RegisterMemberBasket.getInstance().hasPlainText(siger);
				if (success) {
					username = RegisterMemberBasket.getInstance().findPlainText(siger);
				}
			}
			...
			// 注册成员
			List<Node> registerSites = RegisterMemberBasket.getInstance().find(siger);
			for (Node node : registerSites) {
				...
				if (success) {
					success = FrontMemberBasket.getInstance().contains(siger, node);
				}
				if (success) {
			...
			// 在线成员
			List<FrontSeat> onlineSeats = FrontMemberBasket.getInstance().find(siger);

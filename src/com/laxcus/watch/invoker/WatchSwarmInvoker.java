 ...
 * 数据传输速率测试调用器。<br><br>
 * 
 * <B>WATCH站点的这个调用器属于“客户机”。数据流量测试用于检查集群网络的传输速率，选择和确定各节点之间的数据单元尺寸，使集群网络资源达到最大使用效率。</B>
 * 
 * @author scott.liang
 * @version 1.0 8/10/2018
 * @since laxcus 1.0
 */
public class WatchSwarmInvoker extends WatchTrafficInvoker {

	/**
	 * 构造数据传输速率测试调用器，指定命令
	 * @param cmd 数据传输速率测试命令
	 */
	public WatchSwarmInvoker(Swarm cmd) {
		super(cmd);
	}
	...
	public boolean ending() {
		...
		SwarmReflex reflux = null;
		try {
			if (isSuccessObjectable(index)) {
				reflux = getObject(SwarmReflex.class, index);
			}
...
//		Logger.debug(this, "upload", "发送的子包数目:%d - %d", client.getSubPackets(), product.getSendPacket());
		
		Logger.debug(this, "upload", "send subpackets:%d - %d", client.getSubPackets(), product.getSendPacket());

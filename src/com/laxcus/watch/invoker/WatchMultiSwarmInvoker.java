 ...
 * @since laxcus 1.0
 */
public class WatchMultiSwarmInvoker extends WatchInvoker {

	/**
	 * 构造多节点流量测试调用器，指定命令
	 * @param cmd 多节点流量测试命令
	 */
	public WatchMultiSwarmInvoker(MultiSwarm cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		for (int i = 0; i < count; i++) {
			...
			boolean success = WatchCommandPool.getInstance().press(shift);
			if (!success) {
				faults++;
				continue;
			}

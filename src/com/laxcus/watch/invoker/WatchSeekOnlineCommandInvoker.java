 ...
 * @since laxcus 1.0
 */
public class WatchSeekOnlineCommandInvoker extends WatchInvoker {

	/**
	 * 构造检索在线命令调用器，指定命令
	 * @param cmd 检索在线命令
	 */
	public WatchSeekOnlineCommandInvoker(SeekOnlineCommand cmd) {
		super(cmd);
		cmd.setFast(true);
	}
	...
	public boolean ending() {
		SeekOnlineCommandProduct product = null;
		int index = findEchoKey(0);
		try {
			if (isSuccessObjectable(index)) {
				product = getObject(SeekOnlineCommandProduct.class, index);
			}

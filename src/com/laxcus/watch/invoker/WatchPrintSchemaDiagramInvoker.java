 ...
 * @since laxcus 1.0
 */
public class WatchPrintSchemaDiagramInvoker extends WatchPrintResourceDiagramInvoker {

	/**
	 * 构造显示数据库状态的异步调用器，指定命令
	 * @param cmd 显示数据库状态
	 */
	public WatchPrintSchemaDiagramInvoker(PrintSchemaDiagram cmd) {
		super(cmd);
	}
	...
	private void print(List<Account> accounts) {
		...
		for (int index = 0; index < size; index++) {
			...
			if (account == null) {
				...
				List<Schema> array = account.getSchemas();
				if (array.isEmpty()) {
					printEmpty(username);
				} else {
					printSchemas(username, array);
				}
			}
		}
		// 输出全部记录

 ...
 * @since laxcus 1.0
 */
public class SiteOnWatchPool extends VirtualPool {

	/** WATCH节点监视管理池 **/
	private static SiteOnWatchPool selfHandle = new SiteOnWatchPool();

	/** 节点地址 -> 被监视节点 **/
	private Map<Node, WatchTube> sites = new TreeMap<Node, WatchTube>();
	
	/**
	 * 初始化WATCH节点监视管理池
	 */
	private SiteOnWatchPool() {
		super();
	}
	 ...
	 */
	public static SiteOnWatchPool getInstance() {
		return SiteOnWatchPool.selfHandle;
	}
	...
	public List<Node> list(byte siteFamily) {
		...
		super.lockMulti();
		try {
			Iterator<Map.Entry<Node, WatchTube>> iterator = sites.entrySet().iterator();
			while (iterator.hasNext()) {
				Map.Entry<Node, WatchTube> entry = iterator.next();
	...
	public boolean add(Node node) {
		...
		super.lockSingle();
		try {
			WatchTube tube = sites.get(node);
			success = (tube == null);
			if (success) {
				tube = new WatchTube(node);
	...
	public boolean remove(Node node) {
		...
		super.lockSingle();
		try {
			WatchTube tube = sites.remove(node);
			success = (tube != null);
		} catch (Throwable e) {
			Logger.fatal(e);
		} finally {
	 ...
	 * @return 返回实例或者空指针
	 */
	public WatchTube find(Node node) {
		// 锁定！
		super.lockMulti();
		try {
	...
	private void check() {
		...
		if (array.size() > 0) {
			...
			WatchCommandPool.getInstance().admit(cmd);
		}
	}

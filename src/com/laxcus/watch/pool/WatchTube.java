 ...
 * @since laxcus 1.0
 */
public final class WatchTube implements Cloneable, Serializable, Comparable<WatchTube>, Classable {
	...
	private static volatile long timeout = WatchTube.defaultTimeout;
	...
	public static long setMinTimeout(long ms) {
		if (ms >= 1000) {
			WatchTube.minTimeout = ms;
		}
		return WatchTube.minTimeout;
	}
	...
	public static void setTimeout(long ms) {
		if (ms <= 0) {
			WatchTube.timeout = WatchTube.defaultTimeout;
		} else if (ms >= WatchTube.minTimeout) {
			WatchTube.timeout = ms;
		} else {
			WatchTube.timeout = WatchTube.minTimeout;
		}
	}
	 ...
	 */
	private WatchTube(WatchTube that) {
		this();
		node = that.node;
	 ...
	 */
	public WatchTube(ClassReader reader) {
		super();
		resolve(reader);
	}
	...
	public boolean isTimeout() {
		return System.currentTimeMillis() - refreshTime >= WatchTube.timeout;
	}
	 ...
	 * @return WATCH监视站点
	 */
	public WatchTube duplicate() {
		return new WatchTube(this);
	}
	 ...
	 */
	@Override
	public int compareTo(WatchTube that) {
		// 空值在前
		if (that == null) {
			return 1;
		}

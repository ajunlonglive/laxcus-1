 ...
 * @since laxcus 1.0
 */
public class MostCPUParser extends SyntaxParser {

	/** 正则表达式 **/
	private final static String REGEX = "^\\s*(?i)(?:SET\\s+MOST\\s+CPU)\\s+(.+?)\\s+(?i)(?:TO)\\s([\\w\\W]+)\\s*$";

	/**
	 * 构造节点CPU最大使用率解析器
	 */
	public MostCPUParser() {
		super();
	}
	...
	public boolean matches(boolean simple, String input) {
		if (simple) {
			return isCommand("SET MOST CPU", input);
		}
		Pattern pattern = Pattern.compile(MostCPUParser.REGEX);
	 ...
	 * @return 返回MostCPU命令
	 */
	public MostCPU split(String input) {
		// 解析判断
		Pattern pattern = Pattern.compile(MostCPUParser.REGEX);
		Matcher matcher = pattern.matcher(input);
		// 不匹配是错误

 ...
 * @since laxcus 1.0
 */
public final class TimeSector extends Bit32Sector {

	private static final long serialVersionUID = 4760306546655814059L;

	/**
	 * 根据传入参数，构造一个时间分区副本
	 * @param that
	 */
	private TimeSector(TimeSector that) {
		super(that);
	}
	 ...
	 * @since 1.1
	 */
	public TimeSector(ClassReader reader) {
		this();
		resolve(reader);
	}
	 ...
	 * @return - 返回整形值的下标位置
	 */
	private int seekColumn(com.laxcus.access.column.Time column) {
//		CodeScaler scaler = super.createCodeScaler();
	 ...
	 */
	@Override
	public TimeSector duplicate() {
		return new TimeSector(this);
	}

	/**
	 * 根据传入的对象实例，判断它在索引数组中的下标位置。<br>
	 * 允许的对象包括:  <br>
	 * (1) com.laxcus.access.column.Time <br>
	...
	public int indexOf(Object that) {
		...
		if (that.getClass() == com.laxcus.access.column.Time.class) {
			return seekColumn((com.laxcus.access.column.Time) that);
		} else {

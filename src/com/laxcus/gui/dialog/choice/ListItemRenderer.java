...
final class ListItemRenderer extends JLabel implements ListCellRenderer {
	...
	private void loadIcons() {
		fileIcon = UIManager.getIcon("ChoiceDialog.FileIcon");
		folderIcon = UIManager.getIcon("ChoiceDialog.DirectoryIcon");
	}
	...
	public Component getListCellRendererComponent(JList list, Object value,
		...
		if (value.getClass() != FileItem.class) {
			setIcon(null);
			setText("");
			return this;
		}
		...
		if (item.isFile()) {
			...
			String tooltip = String.format("%s<br>%s<br>%s<br>%s", item.path, item.typeDescription, length, lastModified);
			FontKit.setToolTipText(this, tooltip);
		} else if (item.isDirectory()) {
			String tooltip = String.format("%s<br>%s<br>%s", item.path, item.typeDescription, lastModified);
			FontKit.setToolTipText(this, tooltip);
		} else {
			setIcon(null);
			setText("");

 ...
 * @since laxcus 1.0
 */
public class AccountSetSingleMaxJobsInvoker extends AccountInvoker {

	/**
	 * 构造账号最大并行任务数目调用器，指定命令
	 * @param cmd 账号最大并行任务数目
	 */
	public AccountSetSingleMaxJobsInvoker(SetSingleMaxJobs cmd) {
		super(cmd);
	}
	 ...
	 */
	@Override
	public SetSingleMaxJobs getCommand() {
		return (SetSingleMaxJobs) super.getCommand();
	}
	...
	public boolean launch() {
		SetSingleMaxJobs cmd = getCommand();
		...
		if (success) {
			sphere.getAccount().getUser().setJobs(cmd.getJobs());

 ...
 * @since laxcus 1.0
 */
public class AccountOpenShareTableInvoker extends AccountShareCrossInvoker {

	/**
	 * 构造开放数据库资源调用器，制定命令
	 * @param cmd 开放数据库资源
	 */
	public AccountOpenShareTableInvoker(OpenShareTable cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		Logger.debug(this, "launch", success, "share cross items:%d", product.size());
	...
	private Set<CrossFlag> pickup(Account account) {
		...
		if (cmd.isAll()) {
			...
			for (Space define : cmd.getSpaces()) {
				for (Space space : records) {
					if (Laxkit.compareTo(space, define) == 0) {
						array.add(new CrossFlag(space, operator));
					}
				}
			}

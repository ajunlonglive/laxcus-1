 ...
 * @since laxcus 1.0
 */
public class AccountSingleRevokeInvoker extends AccountInvoker {

	/**
	 * 构造单账号解除授权调用器，指定命令
	 * @param cmd 单账号解除授权
	 */
	public AccountSingleRevokeInvoker(SingleRevoke cmd) {
		super(cmd);
	}
	 ...
	 */
	@Override
	public SingleRevoke getCommand() {
		return (SingleRevoke) super.getCommand();
	}
	...
	public boolean launch() {
		...
		if (success) {
			success = sphere.getAccount().revoke(permit);
			if (success) {
		...
		//		// 向账号授权
		//		boolean success = AccountPool.getInstance().revoke(siger, permit);
		...
		Logger.debug(this, "launch", success, "revoke %s", siger);

 ...
 * @since laxcus 1.0
 */
public abstract class AccountDeployCloudPackageInvoker extends AccountInvoker {
	 ...
	 * @param cmd 部署云端应用
	 */
	protected AccountDeployCloudPackageInvoker(DeployCloudPackage cmd) {
		super(cmd);
		step = 1;
		replyFatal = true;
	}
	...
	private boolean replyMailProduct(boolean success, int count) {
		MailCloudPackageProduct product = new MailCloudPackageProduct(success, count);
	...
	private boolean doThird() {
		...
		if (success) {
			// 写入本地
			WareOnAccountPool.getInstance().write(getIssuer(), component);
			// 发送软件包
			refreshPublish();
		}
	...
	protected boolean distributeTaskAssist(Node remote, Naming ware, int phaseFamily, CloudPackageItem item) {
		...
		// 生成工作部件
		TaskPart part = new TaskPart(issuer, phaseFamily);
	...
	protected PlayFruit distribute(int phaseFamily, byte siteFamily, byte siteRank, CloudPackageComponent component) {
		...
		// 1. 更新引导包
		TaskPart part = new TaskPart(getIssuer(), phaseFamily);

 ...
 * @since laxcus 1.0
 */
public class CallMailTaskAssistComponentInvoker extends CallInvoker {

	/**
	 * 构造发布分布任务组件应用附件包
	 * @param cmd 发布分布任务组件应用
	 */
	public CallMailTaskAssistComponentInvoker(MailTaskAssistComponent cmd) {
		super(cmd);
	}
	...
	private boolean reply(boolean success) {
		MailTaskAssistComponent cmd = getCommand();
		MailTaskAssistComponentProduct product = 
			new MailTaskAssistComponentProduct(success, cmd.getFamily());

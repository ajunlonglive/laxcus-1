 ...
 * @since laxcus 1.0
 */
public class CallDropTaskApplicationInvoker extends CallInvoker {

	/**
	 * 构造删除云端应用
	 * @param cmd 删除应用命令
	 */
	public CallDropTaskApplicationInvoker(DropTaskApplication cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		if (!software.toString().matches(Sock.SYSTEM_REGEX)) {
			boolean exists = StaffOnCallPool.getInstance().allow(issuer);
			if (!exists) {
		...
		if (PhaseTag.isInit(family)) {
			taskPool = InitTaskPool.getInstance();
		} else if (PhaseTag.isBalance(family)) {
			taskPool = BalanceTaskPool.getInstance();
		}
		// ESTABLISH 阶段
		else if (PhaseTag.isIssue(family)) {
			taskPool = IssueTaskPool.getInstance();
		} else if (PhaseTag.isAssign(family)) {
			taskPool = AssignTaskPool.getInstance();
		}
		// CONTACT阶段
		else if (PhaseTag.isFork(family)) {
			taskPool = ForkTaskPool.getInstance();
		} else if (PhaseTag.isMerge(family)) {
			taskPool = MergeTaskPool.getInstance();
		}

 ...
 * @since laxcus
 */
public class CallAwardDropSchemaInvoker extends CallInvoker {

	/**
	 * 构造授权删表命令调用器，指定命令
	 * @param cmd 授权删除表
	 */
	public CallAwardDropSchemaInvoker(AwardDropSchema cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 找到全部数据表
		List<Space> spaces = StaffOnCallPool.getInstance().findSpaces(fame);
		// 逐一删除
		for (Space space : spaces) {
			boolean success = StaffOnCallPool.getInstance().dropTable(space);

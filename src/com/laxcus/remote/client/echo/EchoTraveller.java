 ...
 * @since laxcus 1.0
 */
public final class EchoTraveller extends RemoteClient implements EchoClient {
	...
	static {
		try {
			EchoTraveller.methodStart = (EchoVisit.class).getMethod("start",
					new Class<?>[] { EchoFlag.class, EchoHead.class });
			EchoTraveller.methodStop = (EchoVisit.class).getMethod("stop",
					new Class<?>[] { EchoFlag.class, EchoTail.class });
			EchoTraveller.methodPush = (EchoVisit.class).getMethod("push",
					new Class<?>[] { EchoFlag.class, EchoField.class });

			EchoTraveller.methodCast = (EchoVisit.class).getMethod("cast",
					new Class<?>[] { EchoFlag.class, EchoHead.class });
			EchoTraveller.methodExit = (EchoVisit.class).getMethod("exit",
					new Class<?>[] { EchoFlag.class, EchoTail.class });
	...
	//		if (how >= 1) {
	//			EchoExplorer.maxRetry = how;
	//		}
	//		return EchoExplorer.maxRetry;
	//	}
	//
	//	/**
	//	 * 返回最大重传次数
	...
	//		if (timeout >= 1000) {
	//			EchoExplorer.retryInterval = timeout;
	//		}
	//		return EchoExplorer.retryInterval;
	//	}
	//
	//	/**
	//	 * 输出重试间隔时间
	 ...
	 */
	private EchoTraveller(boolean stream) {
	 ...
	 */
	public EchoTraveller(SocketHost endpoint) {
	 ...
	 * @param stream 数据流模式 
	 */
	public EchoTraveller(Cabin hub, boolean stream) {
	 ...
	 * @param hub 目标站点回显地址
	 */
	public EchoTraveller(Cabin hub) {
	...
	public boolean start(EchoFlag flag, EchoHead head) throws VisitException {
		...
		Object param = super.invoke(EchoTraveller.methodStart, params);
	...
	public long push(EchoFlag flag, EchoField field) throws VisitException {
		...
		Object param = super.invoke(EchoTraveller.methodPush, params);
	...
	public boolean stop(EchoFlag flag, EchoTail tail) throws VisitException {
		...
		Object param = super.invoke(EchoTraveller.methodStop, params);
		return ((Boolean) param).booleanValue();
	}
	...
	public CastToken cast(EchoFlag flag, EchoHead head) throws VisitException {
		...
		Object[] params = new Object[] { flag, head };
		Object param = super.invoke(EchoTraveller.methodCast, params);
	...
	public boolean exit(EchoFlag flag, EchoTail tail) throws VisitException {
		...
		Object[] params = new Object[] { flag, tail };
		Object param = super.invoke(EchoTraveller.methodExit, params);
		return ((Boolean) param).booleanValue();
	}
	...
	public boolean post(EchoHead head, EchoHelp tailHelp, ReplySender sender) {
		...
		Logger.note(this, "post", accepted, "connect %s, return token is %s", getRemote(), token);
...
//		Logger.note(this, "post", success, "push to memory, waiting from: %s", getRemote());
		...
		Logger.note(this, "post", success, "sended to: %s", getRemote());
		...
		Logger.note(this, "post", accepted, "closed from: %s", getRemote());

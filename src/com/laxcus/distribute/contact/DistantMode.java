...
/**
 * DISTANT阶段对象和分布任务组件的工作模式 <br>
 * 
 * DISTANT阶段工作模式有两种：产生数据、计算数据。GENERATE、EVALUATE。
 * 
 * @author scott.liang
 * @version 1.0 8/20/2014
 * @since laxcus 1.0
 */
public final class DistantMode {
	...
	/**
	 * 判断是“产生数据”模式
	 * @param who DISTANT阶段模式
	 * @return 返回真或者假
	 */
	public static boolean isGenerate(int who) {
		return who == DistantMode.GENERATE;
	}
	...
	public static boolean isEvaluate(int who) {
		return who == DistantMode.EVALUATE;
	}
	...
	public static boolean isMode(int who) {
		switch (who) {
		case DistantMode.GENERATE:
		case DistantMode.EVALUATE:
			return true;
		default:
			return false;
		}

 ...
 * 快速计算会话<br>
 * 
 * 根据阶段操作需求，包括2种类型：DISTANT/SUBDISTANT。(SUBDISTANT包含在DISTANT中）
 * 
 * @author scott.liang
 * @version 1.2 7/17/2015
 * @since laxcus 1.0
 */
public abstract class ContactSession extends StepSession {
	...
	/** DISTANT会话默认默认命令。通常会有一个，也可以没有，如果有多个命令就保存到自定义参数列表（由用户决定） */
	private Command command;

	/** 索引分区。在INIT/BALANCE阶段产生，是DISTANT阶段分割数据的依据 **/
	 ...
	 * @param family 会话属性
	 */
	protected ContactSession(int family) {
		super(family);
		// 默认无迭代编号，是-1。
		setIterateIndex(-1);
	}
	...
	/**
	 * 根据传入的ContactSession实例，构造它的副本
	 * @param that ContactSession实例
	 */
	protected ContactSession(ContactSession that) {
		super(that);
		// 设置会话迭代序号
	...
	/**
	 * 通过迭代编号判断是子阶段会话((SUBDISTANT，大于0即子级)
	 * @return 返回真或者假
	 */
	...
	/**
	 * 设置索引分区。在INIT/BALANCE阶段设置，在DISTANT阶段结合实际数据使用

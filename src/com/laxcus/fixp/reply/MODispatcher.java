 ...
 * @since laxcus 1.0
 */
public class MODispatcher extends ReplyServer {
	 ...
	 * @param worker 发送代理
	 */
	public MODispatcher(ReplyWorker worker) {
		this();
		setWorker(worker);
	}
	...
	public SocketHost getDefinePrivateHost() {
		...
		return new SocketHost(SocketTag.UDP, definePrivateIP,
				server.getLocalPort());
	}
	...
	public void finish() {
		...
		Logger.info(this, "finish", "exit Massive MO!");
	}
	...
	private final void distribute(DatagramPacket udp) {
		...
		// 检查数据包长度，保存参数
		worker.add(from, udp.getData(), udp.getOffset(), udp.getLength());
	}

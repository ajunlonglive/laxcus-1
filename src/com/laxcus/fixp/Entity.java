...
public class Entity {
	...
	/** FIXP消息集合 **/
	private ArrayList<Message> messages = new ArrayList<Message>(5);
	 ...
	 * @param that Entity实例
	 */
	protected Entity(Entity that) {
		super();
		socketFamily = that.socketFamily;
		remote = that.remote;
		mark = that.mark;
		messages.addAll(that.messages);
		data = that.data;
	}
	...
	private void ratch() {
		if (mark != null) {
			mark.setMessages((short) messages.size());
		}
	}
	...
	public void setData(byte[] b, int off, int len) {
		...
		Message e = new Message(MessageKey.CONTENT_LENGTH, (data == null ? 0 : data.length));
		replaceMessage(e);
	}
	...
	public void setData(Object[] objects) throws IOException {
		byte[] b = PatternConstructor.build(objects);
		if (b == null) {
			throw new IllegalArgumentException("invalid objects");
		}
		setData(b, 0, b.length);
		// 序列化对象宿主(目前只限JAVA语言)
		addMessage(new Message(MessageKey.SERIAL_TYPE, "java"));
		// 数据域的对象成员尺寸
		addMessage(new Message(MessageKey.SERIAL_OBJECTS, objects.length));
	}
	 ...
	 * @return 消息列表，或者空指针
	 */
	public List<Message> findMessages(short key) {
		ArrayList<Message> a = new ArrayList<Message>();
		for (int i = 0; i < messages.size(); i++) {
			Message e = messages.get(i);
			if (e.getKey() == key) {
				a.add(e);
			}
		}
	...
	/**
	 * 查找一个消息，从同值消息序列的0下标开始查找。消息键见MessageKey定义
	 * 
	 * @param key 消息键编号
	...
	public Message findMessage(short key, int index) {
		List<Message> a = findMessages(key);
		// 在范围内
	...
	public byte[] findRaw(short key, int index) {
		Message e = findMessage(key, index);
		if (e == null || !e.isRaw()) {
			return null;
		}
		return e.getRaw();
	 ...
	 * @return 返回新增消息数目
	 */
	public int addMessages(Collection<Message> a) {
	...
	public void addMessage(short key, byte[] value) {
		addMessage(new Message(key, value));
	}
	 ...
	 * @param e 消息实例
	 */
	public void replaceMessage(Message e) {
...
//	}
	...
	public int removeMessage(short key) {
		// 1. 判断匹配键，保存消息
		ArrayList<Message> subs = new ArrayList<Message>();
		for (int index = 0; index < messages.size(); index++) {
			Message e = messages.get(index);
			if (e.getKey() == key) {
				subs.add(e);
	 ...
	 * @return 返回字节数组
	 */
	public byte[] buildHead(int datalen) throws FixpProtocolException {
		replaceMessage(new Message(MessageKey.CONTENT_LENGTH, datalen));
		// 设置消息成员数
		mark.setMessages((short) messages.size());
		// 输出标题到内存
		ClassWriter buff = new ClassWriter(512);
		buff.write(mark.build());
		// 输出消息，输出到内存
	 ...
	 * @return 返回字节数组
	 */
	public byte[] build(int contentSize) throws IllegalValueException {
		if (contentSize < 0) {
			throw new IllegalValueException("content size error %d", contentSize);
		}

		// 设置最新的数据域尺寸(这个尺寸由外部传入，可以是一个提前预定量)
		replaceMessage(MessageKey.CONTENT_LENGTH, contentSize);
		// 设置消息成员数
		mark.setMessages((short) messages.size());
		// 确实实际数据尺寸
	 ...
	 * @return 字节数组
	 */
	public byte[] build() throws IllegalValueException {
	...
	public int getContentLength() {
		Message e = findMessage(MessageKey.CONTENT_LENGTH);
	...
	public void setContentItems(long items) {
		replaceMessage(new Message(MessageKey.CONTENT_ITEMS, items));
	}
	...
	public long getContentItems() {
		Message e = findMessage(MessageKey.CONTENT_ITEMS);

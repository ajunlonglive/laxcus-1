 ...
 * @since laxcus 1.0
 */
public class BankSeekUserAreaInvoker extends BankInvoker {

	/**
	 * 构造检索用户分布区域调用器，指定命令
	 * @param cmd 检索用户分布区域命令
	 */
	public BankSeekUserAreaInvoker(SeekUserArea cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		slaves.addAll(AccountOnBankPool.getInstance().detail());

		// 发送给子节点
		boolean success = (slaves.size() > 0);
		if (success) {
			SeekUserArea sub = getCommand().duplicate();
			int count = incompleteTo(slaves, sub);
			success = (count > 0);
		}
		// 以上不成功，反馈空集
		if (!success) {

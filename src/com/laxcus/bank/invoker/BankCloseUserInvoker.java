 ...
 * @since laxcus 1.0
 */
public class BankCloseUserInvoker extends BankSeekAccountSiteInvoker {
	 ...
	 * @param cmd 删除账号命令
	 */
	public BankCloseUserInvoker(CloseUser cmd) {
		super(cmd);
		step = 1;
	}
	 ...
	 * @param siger 用户签名
	 */
	private void dropToGate(Siger siger) {
	 ...
	 * @param seat
	 */
	private void noticeToHash(Seat seat) {
		RefreshAccount refresh = new RefreshAccount(seat);
		List<Node> slaves = new ArrayList<Node>();
		slaves.addAll(HashOnBankPool.getInstance().detail());
		directTo(slaves, refresh);
	}
	...
	private boolean doThird() {
		...
		if (success) {
			...
			// 通知GATE站点，删除账号
			dropToGate(seat.getSiger());
			// 通知HASH站点，更新站点。注意，是更新非删除！
			noticeToHash(seat);
			...
			CloseUser cmd = getCommand();
			// 反馈结果给GATE
			product = new CloseUserProduct(cmd.getUsername(), false);

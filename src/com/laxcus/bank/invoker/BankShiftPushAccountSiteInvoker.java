 ...
 * @since laxcus 1.0
 */
public class BankShiftPushAccountSiteInvoker extends BankInvoker {

	/**
	 * 构造推送ACCOUNT站点到HASH站点，指定命令
	 * @param shift 推送ACCOUNT站点到HASH站点转发命令
	 */
	public BankShiftPushAccountSiteInvoker(ShiftPushAccountSite shift) {
		super(shift);
	}
	 ...
	 */
	@Override
	public ShiftPushAccountSite getCommand() {
		return (ShiftPushAccountSite) super.getCommand();
	}
	...
	public boolean launch() {
		ShiftPushAccountSite shift = getCommand();

		// 找到全部HASH站点
		List<Node> sites = HashOnBankPool.getInstance().detail();

		// 注册的ACCOUNT站点推送给全部HASH站点

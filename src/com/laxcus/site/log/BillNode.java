 ...
 * @since laxcus 1.0
 */
public final class BillNode implements Serializable, Cloneable, Classable, Comparable<BillNode> {
	 ...
	 * @param that BillNode实例
	 */
	private BillNode(BillNode that) {
		super();
		family = that.family;
		port = that.port;
	}
	 ...
	 * @param port 监听端口
	 */
	public BillNode(byte family, int port) {
		super();
		setFamily(family);
		setPort(port);
	}
	 ...
	 * @param reader 可类化读取器
	 */
	public BillNode(ClassReader reader) {
		super();
		resolve(reader);
	}
	 ...
	 * @return BillNode实例
	 */
	public BillNode duplicate() {
		return new BillNode(this);
	}
	...
	public boolean equals(Object that) {
		if (that == null || that.getClass() != BillNode.class) {
			return false;
		} else if (that == this) {
			return true;
		}

		return compareTo((BillNode) that) == 0;
	}
	 ...
	 */
	@Override
	public int compareTo(BillNode that) {
		// 空对象在前
		if(that == null) {
			return 1;
		}

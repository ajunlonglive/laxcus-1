 ...
 * @since laxcus 1.0
 */
public class HomeCommitReserveResourceInvoker extends HomeInvoker {

	/**
	 * 构造CommitReserveResource命令调用器，指定命令
	 * @param cmd CommitReserveResource命令
	 */
	public HomeCommitReserveResourceInvoker(CommitReserveResource cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		Logger.debug(this, "launch", success, "send to %s", node);
	...
	//
	//		// 把文件移到指定目录下
	//		boolean success = move(source, target);
	//

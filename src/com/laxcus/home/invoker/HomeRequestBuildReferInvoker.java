...
/**
 * 请求分配BUILD站点资源引用调用器。<br>
 * 
 * 这个命令由BUILD站点发出，向HOME站点申请一批账号，以及账号下的数据表名。
 * 
 * @author scott.liang
 * @version 1.0 8/23/2013
 * @since laxcus 1.0
 */
public class HomeRequestBuildReferInvoker extends HomeInvoker {

	/**
	 * 构造请求分配BUILD站点资源引用调用器，指定命令。
	 * @param cmd 请求分配BUILD站点资源引用命令
	 */
	public HomeRequestBuildReferInvoker(RequestBuildRefer cmd) {
		super(cmd);
	}
	 ...
	 */
	@Override
	public RequestBuildRefer getCommand() {
		return (RequestBuildRefer) super.getCommand();
	}
...
//			Vident e = new Vident(username);
//			vidents.put(e.getSiger(), e);
//			// 统计使用次数
//			NodeSet set = BuildOnHomePool.getInstance().findSites(username);
//			if (set != null) {
//				e.addSites(set.size());
//			}
//			// 统计已经使用的数据块尺寸
...
//
//		RequestBuildRefer cmd = getCommand();
//		// 取出BUILD站点的可用内存空间，只使用它的10%，其它空间留给分布计算和数据存储使用
	...
	private RequestReferProduct assign(List<Vident> array) {
		...
		for (Vident vident : array) {
			...
			if (vident.getSites() == 0) {
				...
				// 已经分配的站点达到或者超过指定网关数目时，忽略它
				if (vident.getSites() >= refer.getUser().getBuilders()) {
					Logger.warning(this, "assign", "%s, build site %d > %d",
							siger, vident.getSites(), refer.getUser().getBuilders());
					continue;
				}
				product.add(refer);
			}

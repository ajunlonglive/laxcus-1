 ...
 * @since laxcus 1.0
 */
public class HomeRefreshLoginInvoker extends HomeInvoker {
	 ...
	 * @param cmd 强制重新注册命令
	 */
	public HomeRefreshLoginInvoker(RefreshLogin cmd) {
		super(cmd);
	}
	...
	/**
	 * 来自WATCH站点，HOME站点通知下属站点，重新注册到HOME站点
	 * @param cmd 强制注册命令
	 * @return 成功返回真，否则假
	 */
	private boolean doWatch(RefreshLogin cmd) {
		...
		boolean success = WatchOnHomePool.getInstance().contains(listen);
		// 不成功，拒绝它
		if(!success) {
			return false;
		}
		...
		if (cmd.isAll()) {
			...
			for (Node site : cmd.list()) {
				// 判断是CALL/WORK/BUILD/DATA/LOG中的一种
				success = CallOnHomePool.getInstance().contains(site);
				if (!success) {
					success = WorkOnHomePool.getInstance().contains(site);
				}
				if (!success) {
					success = BuildOnHomePool.getInstance().contains(site);
				}
				if (!success) {
					success = DataOnHomePool.getInstance().contains(site);
				}
				if (!success) {
					success = LogOnHomePool.getInstance().contains(site);
				}
				// 成功，保存它；否则做为失败记录在案
				if (success) {
					sites.add(site);

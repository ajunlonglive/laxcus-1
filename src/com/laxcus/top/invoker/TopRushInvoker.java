...
/**
 * TOP站点的数据块强制转换命令异步调用器。<br>
 * 
 * @author scott.liang
 * @version 1.1 9/2/2012
 * @since laxcus 1.0
 */
public class TopRushInvoker extends TopInvoker {

	/**
	 * 构造数据块强制转换命令异步调用器，指定命令
	 * @param cmd Rush命令
	 */
	public TopRushInvoker(Rush cmd) {
		super(cmd);
	}
	...
	public boolean launch() {
		...
		// 生成副本命令
		Rush sub = new Rush(cmd.getSpace());
		sub.addAll(cmd.list());
		// 以容错模式发送
	...
	public boolean ending() {
		RushProduct product = new RushProduct();

		List<Integer> keys = getEchoKeys();
		for (int index : keys) {
			try {
				if (isSuccessObjectable(index)) {
					RushProduct e = getObject(RushProduct.class, index);
					product.addAll(e);
				}
